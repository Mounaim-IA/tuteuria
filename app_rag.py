...
def extraire_exercice(historique):
    """
    Version Robuste : Cherche l'opération la plus proche de la FIN du message.
    Ignore le texte décoratif et se concentre sur la structure mathématique.
    """
    patterns = [
        (r'(\d+)\s*[\+\+]\s*(\d+)', '+'),
        (r'(\d+)\s*[\-\-]\s*(\d+)', '-'),
    ...
    for msg in reversed(historique):
        if isinstance(msg, AIMessage):
            texte = msg.content
            # On analyse les 4 dernières lignes pour trouver l'exercice posé
...
"""
    ...
    if langue == "العربية":
        ...
        # MISE À JOUR : Redirection vers le menu pour l'arabe
        r6_non = '"أخبره بلطف أن يستخدم القائمة المنسدلة في أعلى اليمين لاختيار الدرس الجديد. هذه هي الطريقة التقنية الوحيدة لتغيير الدرس. 📚"'
        ...
        # MISE À JOUR : Redirection vers le menu pour le français
        r6_non = '"Dis-lui gentiment d\'utiliser le menu déroulant en haut à gauche pour sélectionner le nouveau chapitre. C\'est le seul moyen technique pour lui de changer de leçon. 📚"'
...
1. ACCUEIL ET EXPLICATION (R1, R2, R3) :Élève : "je veux apprendre l'addition" 
Tuteur : "Bonjour ! 😊 C'est une excellente idée. 📖 L'addition, c'est comme regrouper des objets. Si tu as 5 oranges et que j'en ajoute 3, tu en auras 8. Ce résultat s'appelle la Somme. Tu as compris ? 😊 On essaie ! ✏️ À toi ! Combien font 5 + 2 ? 😊"
2. CORRECTION RIGOUREUSE (R4) :Élève : "7 + 5 ça fait 11"
Tuteur : "👏 C'est bien d'avoir essayé ! La bonne réponse est 12, pas 11. Ne t'inquiète pas, c'est en faisant des erreurs qu'on apprend ! 😊💪 Regarde : 7 plus encore 3, cela fait déjà 10. Il en reste encore 2 à ajouter, ce qui fait bien 12 ! ✏️ On tente un autre ? Combien font 4 + 3 ? 😊"
3. RÉSULTAT NÉGATIF ET ARRÊT (R4) :Élève : "Combien font 3 - 10 ?"Tuteur : "👏 Quelle bonne curiosité ! Mais pour l'instant, on travaille seulement avec des résultats positifs 😊 Tu découvriras cela plus tard au collège ! 📚 Essaie avec un plus grand nombre en premier ! 💪" (STOP)
4. CONFIRMATION DE CHAPITRE (R7) :Élève : "Je veux faire des multiplications" (En session 'Addition')Tuteur : "Je vois que tu t'intéresses à la Multiplication ! 😊 Pour l'instant, nous étudions l'Addition. Veux-tu finir cette leçon d'abord ou préfères-tu choisir 'Multiplication' dans le menu pour changer de sujet ? 📚"
5. GESTION DES ERREURS DE SAISIE (R6) :Élève : "8 +"Tuteur : "😊 Je vois que tu as commencé un calcul ! Il manque un nombre. Par exemple, écris 8 + 4. Complète ton opération et on continue ! 💪"
"""
...
def detecter_signe_incompatible(message: str, chapitre_cle: str, langue: str):
    ...
    Utilise des Regex pour éviter les faux positifs (ex: le 'x' de 'veux').
    """
    import re as re_sec
    msg = message.lower().strip()
    
    # 1. LISTE DE PERMISSION (Bypass)
    # On autorise les mots de navigation et les réponses de confirmation
    mots_ok = [
    ...
    if any(m in msg for m in mots_ok):
        return None # On laisse GPT gérer la réponse pédagogique

    # 2. DÉFINITION DES MOTIFS MATHÉMATIQUES (Regex)
    # On cherche le signe uniquement s'il est entouré de chiffres ou d'espaces
    # Cela évite de détecter le 'x' à l'intérieur d'un mot comme 'veux'
    DICO_REGEX = {
    ...
    for chap, pattern in DICO_REGEX.items():
        ...
        # Si on trouve un motif d'un AUTRE chapitre
        if re_sec.search(pattern, msg):
            # Traduction des noms de chapitres pour le message
