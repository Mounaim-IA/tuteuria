...
<style>
    ...
    .header-container {
        /* On utilise le dégradé turquoise du bouton Exercice */
        background: linear-gradient(135deg, #4ECDC4, #45B7D1) !important; 
        
        /* On enlève la bordure rouge pour un style plus propre */
   ...
   /* ══ FORMULAIRE DÉMARRAGE ══ */
    .start-form {
        ...
        border: 2px solid #FF6B6B; /* Bordure orange/corail */
        border-radius: 16px;
        padding: 14px 20px;
        margin-bottom: 14px;
    ...
    .start-form h3 {
        font-family: 'Fredoka One', cursive;
        font-size: 0.95rem;
        /* On enlève la couleur ici car elle est gérée dans le HTML plus bas */
        margin: 0;
    }
    .eleve-info-bar {
...
if not chat_actif:
    ...
    if langue_choisie == "Français":
        titre_html = (
            "<b style=\"font-size:1.05rem; color:#FF6B6B;\">🧮 Bienvenue !</b> "
            "<span style=\"color:#FF6B6B;\">Je suis ton tuteur IA en mathématiques.</span><br>"
            "<span style=\"font-size:0.87rem; font-weight:600; color:#FF8E8E;\">"
            "Veuillez écrire ton prénom, sélectionner un niveau et cliquer sur Commencer."
            "</span>"
        )
    else:
        titre_html = (
            "<b style=\"font-size:1.05rem; color:#FF6B6B;\">🧮 مرحباً !</b> "
            "<span style=\"color:#FF6B6B;\">أنا معلمك الذكي في الرياضيات.</span><br>"
            "<span style=\"font-size:0.87rem; font-weight:600; color:#FF8E8E;\">"
