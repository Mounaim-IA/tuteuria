...
if not chat_actif:
    ...
    col_prog, col_vide = st.columns([1, 1])
    with col_prog:
        st.markdown(f"""
        <div style="margin:4px 0 8px 0; padding:6px 10px;
                    background:linear-gradient(135deg,#f0fffe,#e8f5ff);
                    border-radius:10px; border:1px solid #4ECDC4;">
            <div style="display:flex; justify-content:space-between; margin-bottom:3px;">
                <span style="font-size:11px; color:#0F6E56; font-weight:600;">{lbl_progression}</span>
                <span style="font-size:11px; color:#555;">{lbl_score_prog}</span>
            </div>
            <div style="background:#e0e0e0; border-radius:8px; height:8px; overflow:hidden;">
                <div style="background:linear-gradient(90deg,#4ECDC4,#0F6E56);
                            width:{progression_pct}%; height:100%; border-radius:8px;"></div>
            </div>
            <div style="text-align:center; margin-top:2px;">
                <span style="font-size:10px; color:#0F6E56; font-weight:600;">✨ {etape_lbl_actuelle}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
