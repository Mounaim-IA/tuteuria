            ...
            with st.spinner(t["thinking"]):
                try:
                    ...
                    if _img_path and _choix_consigne and etape_actuelle == "explication":
                        ...
                        # 3. Forcer GPT à utiliser ce texte exact
                        if langue_choisie == "العربية":
                            consigne_specifique = (
                                f"[تعليمات النظام : تم عرض صورة تعليمية. "
                                f"✅ إلزامي جداً : يجب عليك أن تبدأ إجابتك بالحرف الواحد بهذا النص :\n"
                                f"« {explication_statique} »\n"
                                f"بعد كتابة هذا النص بالضبط، لا تضف أي شرح آخر من عندك. فقط اختم بـ '✏️ دورك !' مع تمرين جديد كلياً من نوع : {_choix_consigne} "
                                f"بأرقام مختلفة.]"
                            )
                        else:
                            consigne_specifique = (
                                f"[CONSIGNE SYSTÈME : Une image pédagogique est affichée. "
                                f"✅ OBLIGATOIRE : Tu DOIS commencer ta réponse EXACTEMENT mot pour mot par ce texte :\n"
                                f"« {explication_statique} »\n"
                                f"Après avoir écrit ce texte, n'ajoute AUCUNE autre explication de ton côté. Termine directement par '✏️ À toi !' avec UN NOUVEL EXERCICE de type : {_choix_consigne}. "
                                f"Invente de nouveaux nombres pour l'exercice.]"
                            )
