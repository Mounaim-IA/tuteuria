...
def extraire_exercice(historique):
    """
    Extrait une expression mathématique complète (ex: 5 - 7 + 9) 
    au lieu de s'arrêter aux deux premiers nombres.
    """
    # On parcourt l'historique en partant de la fin
    for msg in reversed(historique):
        if isinstance(msg, AIMessage):
            texte = msg.content
            # On cherche la position du dernier exercice posé (✏️ ou 🎯)
            pos = -1
            if "✏️" in texte:
                pos = texte.rindex("✏️")
            elif "🎯" in texte:
            ...
            if pos >= 0:
                # On analyse uniquement le texte après l'emoji
                texte_exercice = texte[pos:]
                
                # Ce Regex capture une suite de nombres séparés par +, -, *, x, ÷
                # Il peut lire "5 + 3", mais aussi "5 - 7 + 9"
                match = re.search(r'(\d+(?:\s*[+\-*x÷]\s*\d+)+)', texte_exercice)
                
                if match:
                    expression_brute = match.group(1)
                    # Nettoyage pour que Python puisse calculer (x devient *, ÷ devient /)
                    calcul_python = expression_brute.replace('x', '*').replace('÷', '/')
                    
                    try:
                        # On calcule le résultat réel de l'expression complète
                        resultat_attendu = eval(calcul_python)
                        
                        # On retourne un tuple de 4 éléments pour rester compatible 
                        # avec le reste de ton code (a, op, b, résultat)
                        # On met l'expression entière dans la première case
                        return (expression_brute, "mixte", "mixte", resultat_attendu)
                    except:
                        return None
            break
    return None
...
🔴 RÈGLE DES OPÉRATIONS MIXTES (ex: 6 - 8 + 2) :
Si la première soustraction est impossible (6 < 8), interdiction de faire l'addition de droite en premier. 
Tu DOIS utiliser la méthode de la PERMUTATION :
1. On déplace les nombres pour mettre les "plus" ensemble : 6 - 8 + 2 devient (6 + 2) - 8.
2. Étape 1 : 6 + 2 = 8.
3. Étape 2 : 8 - 8 = 0.
✅ Résultat final = 0. 
Explique toujours cette permutation comme un "déplacement de jetons" pour que l'enfant comprenne.
🔴 Décimaux :
