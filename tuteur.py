                ...
                if matches:
                    ...
                    try:
                        ...
                        # ── DIVISION EUCLIDIENNE : jamais de décimal au primaire ──
                            ...
                            import re as _re
                            _parts = calcul_python.split('/')
                            if len(_parts) == 2:
                                try:
                                    _a = int(float(_parts[0].strip()))
                                    _b = int(float(_parts[1].strip()))
                                    _q = _a // _b
                                    _r = _a % _b
                                    resultat = f"{_q} reste {_r}" if _r != 0 else _q
                                except Exception:
                                    pass
...
def post_traitement(reply, user_input, historique, langue):
    ...
    mots_fr = ['bravo', 'correct', 'exact', 'parfait', 'excellent',
                'très bien', 'super', 'juste',
...
def get_image_for_calcul(a, op, b, niveau):
    ...
    # --- CORRECTION : Neutraliser les images pour les décimaux ---
    if is_decimal:
        noms_op = {'+': 'addition', '-': 'soustraction', '−': 'soustraction', 
                   '×': 'multiplication', '*': 'multiplication', 'x': 'multiplication', 
                   '÷': 'division', '/': 'division'}
        nom = noms_op.get(op, 'opération')
        return None, f"{nom} de nombres décimaux"
    # -------------------------------------------------------------
...
→ Si INCORRECT — SÉQUENCE OBLIGATOIRE EN 4 TEMPS :
   ...
   Explique POURQUOI la réponse est fausse. Adapte la méthode au type d'opération :
   ...
   "Posons le calcul ensemble ! On met les unités sous les unités et les dizaines sous les dizaines."
   Puis explique colonne par colonne avec les VRAIS chiffres de l'exercice :
   
   - Addition SANS retenue : "Les unités : 4 + 3 = 7. Les dizaines : 2 + 1 = 3. Résultat : 37."
   - Addition AVEC retenue : "Les unités : 7 + 5 = 12. On écrit 2 et on retient 1. Les dizaines : 2 + 3 + 1 (retenue) = 6. Résultat : 62."
   - Soustraction SANS emprunt : "Les unités : 8 - 3 = 5. Les dizaines : 4 - 2 = 2. Résultat : 25."
   - Soustraction AVEC emprunt : "Les unités : 3 est plus petit que 8, donc on emprunte 1 dizaine. 13 - 8 = 5. Les dizaines : 4 - 1 (emprunt) - 2 = 1. Résultat : 15."
   - Soustraction DOUBLE EMPRUNT : "Les unités : 0 est plus petit que 7, on emprunte aux dizaines. Mais les dizaines valent 0 aussi, donc on emprunte d'abord aux centaines. Les centaines perdent 1, les dizaines deviennent 9 (car on re-prête 1 aux unités), les unités deviennent 10."
   - Multiplication SIMPLE (1×2 ch) : "Les unités : 4 × 3 = 12. On écrit 2 et on retient 1. Les dizaines : 2 × 3 = 6 + 1 (retenue) = 7. Résultat : 72."
   - Multiplication 2×2 CHIFFRES : "On fait deux lignes. Ligne 1 : on multiplie tout par les unités du deuxième nombre. Ligne 2 : on multiplie tout par les dizaines et on décale d'un cran. Puis on additionne les deux lignes."
...
⚠️ RÈGLE ABSOLUE — VERDICT PYTHON :
   ...
   Tu ne dois JAMAIS le contredire ou faire ton propre calcul.
    ...
    → Si Si pas de verdict : l'élève n'a pas répondu à un exercice, traite normalement.
