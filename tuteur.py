...
with col_p:
    if st.button(lbl_print, use_container_width=True, key="btn_download_conv"):
        ...
        # Sauvegarder la durée dans Supabase
        inf_dl = st.session_state[eleve_key]
        db_maj_session(
            session_id    = inf_dl.get("session_db_id"),
            bonnes        = sc_d["bonnes"],
            total         = sc_d["total"],
            nb_messages   = len(st.session_state[session_key]),
            etape_finale  = st.session_state.get(etape_key, "amorce"),
            duree_minutes = duree_d,
        )

        # ── Générer le PDF avec fpdf2 ─────────────────────────
        def nettoyer_texte(txt):
            """Supprime les emojis et balises non supportés par fpdf."""
        ...
        # En-tête coloré
        pdf.set_fill_color(255, 107, 107)   # coral
        ...
        # Score
        pdf.set_y(55)
        pdf.set_fill_color(240, 255, 254)
        pdf.set_draw_color(78, 205, 196)
        pdf.set_text_color(15, 110, 86)
        pdf.set_font("Helvetica", "B", 12)
        pdf.set_x(15)
        pdf.cell(180, 10,
                 f"Score : {sc_d['bonnes']}/{sc_d['total']} - Taux de reussite : {taux_d}%",
        ...
        for msg in st.session_state[session_key]:
            ...
            texte    = nettoyer_texte(msg.content)
            if not texte: continue

            if is_eleve:
                # Bulle élève (droite, fond violet clair)
                pdf.set_fill_color(238, 237, 254)
                pdf.set_font("Helvetica", "B", 9)
                pdf.set_x(100)
                pdf.cell(95, 6, "Eleve", ln=True, align="R")
                pdf.set_font("Helvetica", "", 9)
                for ligne in texte.split("\n"):
                    if ligne.strip():
                        pdf.set_x(100)
                        pdf.multi_cell(95, 5, ligne.strip(), fill=True, align="R")
            else:
                # Bulle tuteur (gauche, fond teal clair)
                pdf.set_fill_color(225, 245, 238)
                pdf.set_font("Helvetica", "B", 9)
                pdf.set_x(15)
                pdf.cell(95, 6, "Tuteur IA", ln=True)
                pdf.set_font("Helvetica", "", 9)
                for ligne in texte.split("\n"):
                    if ligne.strip():
                        pdf.set_x(15)
                        pdf.multi_cell(95, 5, ligne.strip(), fill=True)
            pdf.ln(2)
