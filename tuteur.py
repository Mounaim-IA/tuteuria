        ...
        with st.chat_message("assistant"):
            with st.spinner(t["thinking"]):
                try:
                    ...
                    if _verdict_pre and _verdict_pre.startswith("incorrect"):
                    ...
                    if st.session_state.get("erreurs_consec", 0) >= 2:
                        if langue_choisie == "العربية":
                            alerte = "\n\n[تنبيه مهم : التلميذ أخطأ 3 مرات متتالية ! ❌ لا تتركه محبطاً. أعطه النتيجة النهائية لعمليته فوراً مع تشجيع لطيف، ثم اطرح تمريناً جديداً كلياً ✏️]"
                        else:
                            alerte = "\n\n[ALERTE : L'élève a fait 3 erreurs consécutives ! ❌ Évite la frustration : DONNE LA RÉPONSE FINALE de son calcul, explique doucement, puis pose un NOUVEL EXERCICE ✏️]"
                        
                        if consigne_specifique:
                            consigne_specifique += alerte
                        else:
                            consigne_specifique = alerte
