...
def generer_menu_choix(operation, niveau, langue):
    ...
    Retourne (texte_menu, liste_choix) ou (None, None) si pas de choix.
    """
    niv_num = int(str(niveau).replace("CE", "")) if "CE" in str(niveau) else 3
    choix_list = CHOIX_OPERATIONS.get(operation, {}).get(niv_num)

    if not choix_list:
        # Essayer le niveau inférieur
        for n in range(niv_num, 0, -1):
            choix_list = CHOIX_OPERATIONS.get(operation, {}).get(n)
            if choix_list:
                break
    if not choix_list:
        return None, None
    ...
    if len(choix_list) == 1:
        # Un seul choix → pas besoin de menu, on l'affiche directement
        return None, choix_list
    ...
    consignes = {
        ...
        "explication": {
            "Français": "[CONSIGNE SYSTÈME : Tu es en phase EXPLICATION. "
                        "IMPORTANT : Une image pédagogique a été affichée au-dessus de ta réponse. "
                        "Cette image montre DÉJÀ la décomposition verticale complète avec les couleurs et les étapes. "
                        "NE RÉPÈTE PAS ce que l'image montre ! Ne fais PAS de décomposition verticale en texte. "
                        "Ton rôle : 1) Dis une phrase d'accroche concrète courte (ex: 'L'addition, c'est comme compter des bonbons 🍬') "
                        "2) Dis : 'Regarde bien l'image ci-dessus, elle t'explique la méthode pas à pas ! 😊' "
                        "3) Donne UNE stratégie mentale (ex: 'Mets le grand nombre dans ta tête et compte sur tes doigts') "
                        "4) Termine par '✏️ À toi !' avec UN exercice adapté au niveau, avec des nombres DIFFÉRENTS de l'image. "
                        "Ta réponse doit être COURTE (5-6 lignes max).]",
            "العربية": "[تعليمات النظام : أنت في مرحلة الشرح. "
                       "مهم : تم عرض صورة تعليمية فوق إجابتك. "
                       "هذه الصورة تُظهر بالفعل التفكيك العمودي الكامل بالألوان والخطوات. "
                       "لا تكرر ما تظهره الصورة ! لا تقم بالتفكيك العمودي في النص. "
                       "دورك : 1) جملة ربط حسي قصيرة (مثال: 'الجمع مثل عدّ الحلويات 🍬') "
                       "2) قل : 'انظر جيدا للصورة أعلاه، فهي تشرح لك الطريقة خطوة بخطوة ! 😊' "
                       "3) أعطِ استراتيجية ذهنية واحدة (مثال: 'ضع العدد الكبير في رأسك وعُدّ على أصابعك') "
                       "4) اختم بـ '✏️ دورك !' مع تمرين واحد مناسب للمستوى، بأرقام مختلفة عن الصورة. "
...
if user_input:
    ...
    if etape_actuelle == "amorce":
        ...
        if operation:
            ...
            menu_text, choix_list = generer_menu_choix(operation, niveau_eleve, langue_choisie)
        ...
        with st.spinner(t["thinking"]):
            try:
                ...
                if _img_path and _choix_consigne and etape_actuelle == "explication":
                    # Consigne spécifique : image affichée + type d'exercice précis
                    if langue_choisie == "العربية":
                        consigne_specifique = (
                    ...
                    _img_path = None  # Pas d'image si pas de choix
