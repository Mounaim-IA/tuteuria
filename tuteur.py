...
def detecter_image_operation(message, reponse_tuteur, niveau, langue):
    ...
    # ── Condition 1 : on affiche l'image SEULEMENT pendant l'explication ──
    est_explication = any(mot in rep for mot in [
        "technique", "méthode", "voici comment", "regarde",
        "imagine", "mets", "dans ta tête", "comprends",
        "tu as compris", "essaie", "à toi",
        "تقنية", "طريقة", "تخيل", "انظر", "هل فهمت",
        "ضع", "في رأسك", "دورك", "جرب",
    ])
    ...
    if not (est_explication or demande_apprentissage):
        return None
    ...
    # --- Addition ---
    if any(mot in combined for mot in ["addition", "additionner", "ajouter", "الجمع", "يجمع"]):
        if niv_num <= 1:
            return images.get("addition_simple")
        if any(mot in combined for mot in ["décimal", "virgule", "عشري", "فاصلة"]):
            return images.get("addition_decimaux")
        if any(mot in combined for mot in ["3 chiffres", "centaine", "ثلاث", "مئ"]):
            return images.get("addition_3_chiffres")
        if any(mot in combined for mot in ["retenue", "احتفاظ"]) or niv_num >= 2:
            return images.get("addition_avec_retenue")
        return images.get("addition_sans_retenue")

    # --- Soustraction ---
    if any(mot in combined for mot in ["soustraction", "soustraire", "enlever", "retirer", "الطرح", "يطرح"]):
        if niv_num <= 1:
            return images.get("soustraction_simple")
        if any(mot in combined for mot in ["décimal", "virgule", "عشري", "فاصلة"]):
            return images.get("soustraction_decimaux")
        if any(mot in combined for mot in ["double emprunt", "3 chiffres", "centaine", "استلافين", "مئ"]):
            return images.get("soustraction_double_emprunt")
        if any(mot in combined for mot in ["emprunt", "emprunte", "استلاف"]) or niv_num >= 2:
            return images.get("soustraction_avec_retenue")
        return images.get("soustraction_sans_retenue")

    # --- Multiplication ---
    if any(mot in combined for mot in ["multiplication", "multiplie", "fois", "الضرب", "يضرب", "×"]):
        if any(mot in combined for mot in ["table", "جدول"]):
            return images.get("tables_multiplication")
        if any(mot in combined for mot in ["10", "100", "1000"]):
            return images.get("multiplication_10_100_1000")
        if any(mot in combined for mot in ["décimal", "virgule", "عشري"]):
            return images.get("multiplication_decimaux")
        if any(mot in combined for mot in ["fraction", "كسر"]):
    ...
    # --- Division ---
    if any(mot in combined for mot in ["division", "divise", "partage", "القسمة", "يقسم", "÷"]):
        if any(mot in combined for mot in ["décimal", "virgule", "عشري"]):
            if any(mot in combined for mot in ["diviseur", "المقسوم عليه"]):
                return images.get("division_diviseur_decimal")
            return images.get("division_decimale")
        if any(mot in combined for mot in ["fraction", "كسر"]):
            return images.get("division_fractions")
        if niv_num <= 3:
            if any(mot in combined for mot in ["reste", "الباقي"]):
                return images.get("division_avec_reste")
            return images.get("division_simple")
        return images.get("division_2_chiffres")

    # --- Fractions ---
    if any(mot in combined for mot in ["fraction", "كسر", "كسور", "moitié", "quart", "tiers",
                                        "نصف", "ربع", "ثلث"]):
        if any(mot in combined for mot in ["équival", "متكافئ"]):
            return images.get("fractions_equivalentes")
        if any(mot in combined for mot in ["simplif", "تبسيط", "pgcd"]):
            return images.get("simplification_fractions")
        if any(mot in combined for mot in ["compar", "مقارنة"]):
            return images.get("comparaison_fractions")
        if any(mot in combined for mot in ["décimal", "عشري", "conversion", "تحويل", "0,5", "0,25"]):
            return images.get("fractions_decimales")
        if any(mot in combined for mot in ["multipli", "ضرب"]) and "fraction" in combined:
            return images.get("multiplication_fractions")
        if any(mot in combined for mot in ["divis", "قسم"]) and "fraction" in combined:
            return images.get("division_fractions")
        if any(mot in combined for mot in ["additionn", "جمع", "ajouter"]):
            if any(mot in combined for mot in ["différent", "مختلف"]):
                return images.get("fractions_denom_diff")
            return images.get("addition_fractions")
        if any(mot in combined for mot in ["soustrai", "طرح"]):
            return images.get("soustraction_fractions")
        if any(mot in combined for mot in ["nombre", "عدد", "de "]):
            return images.get("fraction_d_un_nombre")
        return images.get("fractions_introduction")

    # --- Double / Moitié ---
    if any(mot in combined for mot in ["double", "moitié", "الضعف", "النصف"]):
        return images.get("double_moitie")

    # --- Priorité des opérations ---
    if any(mot in combined for mot in ["priorité", "أولوية", "d'abord", "أولا"]):
        return images.get("priorite_operations")

    # --- Multiples et diviseurs ---
    if any(mot in combined for mot in ["multiple", "diviseur", "مضاعف", "قاسم"]):
        return images.get("multiples_diviseurs")

    # --- Opérations mixtes ---
    if any(mot in combined for mot in ["mixte", "مختلط"]):
        return images.get("operations_mixtes")

    # --- Numération ---
    if any(mot in combined for mot in ["numération", "centaine", "dizaine", "unité",
                                        "ترقيم", "مئة", "عشرة", "وحدة"]):
    ...
    if etape_actuelle == "amorce":
        # Si le tuteur commence à expliquer (ancrage concret, décomposition...)
...
def injecter_consigne_etape(message, etape, langue):
    ...
    consignes = {
        ...
        "explication": {
            "Français": "[CONSIGNE SYSTÈME : Tu es en phase EXPLICATION. Fais l'ancrage concret + décomposition verticale + stratégie mentale. Termine par '✏️ À toi !' avec un exercice.]",
            "العربية": "[تعليمات النظام : أنت في مرحلة الشرح. قدم الربط الحسي + التفكيك العمودي + الاستراتيجية الذهنية. اختم بـ '✏️ دورك !' مع تمرين.]"
        },
        "exercice1": {
...
for i, msg in enumerate(chat_history):
    ...
    with st.chat_message(role):
        # Si c'est un message tuteur, vérifier si une image doit être affichée
        if role == "assistant" and i > 0 and isinstance(chat_history[i-1], HumanMessage):
            _img_path = detecter_image_operation(
                chat_history[i-1].content, msg.content,
                eleve_info.get("niveau", ""), langue_choisie
            )
    ...
    with st.chat_message("assistant"):
        with st.spinner(t["thinking"]):
            try:
                ...
                # ── 2. Machine à états : Injection de la consigne (Le Marionnettiste) ──────
                etape_actuelle = st.session_state[etape_key]
                message_final = injecter_consigne_etape(message_avec_verdict, etape_actuelle, langue_choisie)

                response        = (prompt | llm).invoke({
                    "input": message_final,  # <-- On utilise message_final ici !
                ...
                # ── 3. Machine à états : Mise à jour de l'étape (Le Cœur) ──────
                ...
                # ── 4. Image pédagogique (Étape 2 : explication) ──────
                _img_path = detecter_image_operation(
