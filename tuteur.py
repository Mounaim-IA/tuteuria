...
def extraire_exercice(historique):
    ...
    for msg in reversed(historique):
        if isinstance(msg, AIMessage):
            ...
            # Chercher TOUS les calculs présents dans le message de l'IA
            matches = list(re.finditer(r'(\d+(?:[,\.]\d+)?(?:\s*[+\-×*x÷/]\s*\d+(?:[,\.]\d+)?)+)', texte))
            
            if matches:
                # Par défaut, on prend le tout dernier calcul mentionné par le tuteur
                expression_brute = matches[-1].group(1).strip()
                
                # Priorité : Si un calcul est près d'un mot-clé (✏️, 🤔, font, يساوي), on le cible en priorité
                for m in matches:
                    idx = m.start()
                    contexte_avant = texte[max(0, idx-25):idx].lower()
                    if any(mot in contexte_avant for mot in ["✏️", "🤔", "font", "يساوي", "calcule"]):
                    ...
                    if st.session_state.get("erreurs_consec", 0) >= 2:
                        if langue_choisie == "العربية":
                            alerte_frustration = (
                                ...
                                "قم بتطبيق خطوات الصورة لحل عملية التلميذ الأصلية واشرحها خطوة بخطوة (كيف نضع العملية، كيف نستلف، إلخ باستخدام أرقامه). "
                                "أعطِ النتيجة النهائية بوضوح، ثم اطرح تمريناً جديداً ✏️]"
                            )
                        else:
                            alerte_frustration = (
                                "\n\n[CONSIGNE SYSTÈME - PRIORITÉ ABSOLUE : L'élève a fait 2 erreurs ! ❌ "
                                "IGNORE la règle 'NE DONNE JAMAIS LA RÉPONSE'. "
                                "APPLIQUE les étapes de l'image pour résoudre le calcul original de l'élève. "
                                "Explique ÉTAPE PAR ÉTAPE avec SES chiffres (comment poser l'opération, comment emprunter, etc.). "
                                "Donne le résultat final clairement, puis pose un NOUVEL EXERCICE ✏️]"
                            )
