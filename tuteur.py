...
(function() {{
    function styleBtns() {{
        ...
        doc.querySelectorAll('button').forEach(function(btn) {{
            ...
            // 1. LE SECRET : Forcer la boîte cachée de Streamlit à prendre 100% de la colonne
            ...
            // 2. Style des boutons optimisé pour mobile
            btn.setAttribute('style',
                ...
                'width:100% !important;' +
                'white-space:nowrap !important;' + /* Garde le texte sur une ligne */
                'overflow:hidden !important;' +
                'text-overflow:ellipsis !important;' /* Ajoute "..." si l'écran est vraiment minuscule */
            );
            
            btn.querySelectorAll('p, span, div').forEach(function(el) {{
                el.style.cssText = 'margin:0 !important;padding:0 !important;text-align:center !important;width:100% !important;color:white !important;font-size:inherit !important;white-space:nowrap !important;overflow:hidden !important;text-overflow:ellipsis !important;';
            }});
        ...
        // 3. Répartition mathématique parfaite (50% - 3px)
        if (targetRow) {{
            ...
            targetRow.style.setProperty('gap', '6px', 'important'); /* Espace de 6px au centre */
            targetRow.style.setProperty('width', '100%', 'important');
            
            var cols = targetRow.querySelectorAll('div[data-testid="column"]');
            cols.forEach(function(c) {{
                // Calcul exact : La moitié de l'écran MOINS la moitié de l'espace central.
                // Ainsi : (50% - 3px) + (50% - 3px) + 6px (gap) = EXACTEMENT 100%. 
                c.style.setProperty('width', 'calc(50% - 3px)', 'important');
                c.style.setProperty('min-width', '0', 'important');
                c.style.setProperty('flex', '0 0 calc(50% - 3px)', 'important');
                c.style.setProperty('padding', '0', 'important');
                c.style.setProperty('margin', '0', 'important');
            }});
        }}
    }}
    // Exécution rapide pour éviter tout "saut" visuel au chargement
