                    ...
                    try:
                        ...
                        if _img_path and _choix_consigne and etape_actuelle in ("explication", "choix_sujet"):
                            ...
                            if etape_pour_consigne in ("exercice2", "quiz"):
                                try:
                                    _exo_ref = (extraire_exercice(chat_history) or "").replace('−', '-')
                                    if "-" in _exo_ref:
                                    ...
                                    def _res_calc(expr):
                                        try:
                                            ...
                                            return None
                                    _res_prec = _res_calc(_exo_ref) if _exo_ref else None
                                    # Générer un exercice cohérent (emprunt/retenue) ET de résultat différent
                                    _calcul_force = None
                                    if _exo_ref:
                                        _calcul_force = generer_modelage((_exo_ref,))
                                    if not _calcul_force:
                                        _calcul_force = _generer_avec_protection(_op_ref, _cons_ref or "", _niv_ref)
                                        for _ in range(15):
                                            if _res_prec is None or _res_calc(_calcul_force) != _res_prec:
                                                break
                                            _calcul_force = _generer_avec_protection(_op_ref, _cons_ref or "", _niv_ref)
                                except Exception:
                                    _calcul_force = None
