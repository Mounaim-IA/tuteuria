    ...
    for msg in reversed(historique):
        if isinstance(msg, AIMessage):
            ...
            if "🎯" in texte:
                pos = texte.rindex("🎯")
            if "✏️" in texte:
                pos = max(pos, texte.rindex("✏️"))
            if "🤔" in texte:
                pos = max(pos, texte.rindex("🤔"))
            if "Combien font" in texte:
                pos = max(pos, texte.rindex("Combien font"))
            if "كم يساوي" in texte:
                pos = max(pos, texte.rindex("كم يساوي"))
            
            # Si aucun mot-clé n'est trouvé, on cherche le calcul dans tout le texte par sécurité
            if pos == -1:
                pos = 0
...
def get_image_for_calcul(a, op, b, niveau, langue):
    ...
    """
    # On utilise directement le dictionnaire de la langue choisie
    ...
    with st.chat_message(role):
        if role == "assistant":
            ...
            if _img_stored and os.path.exists(_img_stored):
                st.image(_img_stored, width=250)
        ...
        with st.chat_message("assistant"):
            with st.spinner(t["thinking"]):
                try:
                    ...
                    if _verdict_pre and isinstance(_verdict_pre, str) and _verdict_pre.startswith("incorrect"):
                        st.session_state["erreurs_consec"] = st.session_state.get("erreurs_consec", 0) + 1
                    elif _verdict_pre == "correct" or (isinstance(_verdict_pre, str) and _verdict_pre.startswith("presque")):
                        st.session_state["erreurs_consec"] = 0

                    if st.session_state.get("erreurs_consec", 0) >= 2:
                        if langue_choisie == "العربية":
                            alerte_frustration = (
                                "\n\n[تعليمات النظام - أولوية قصوى : التلميذ أخطأ مرتين متتاليتين ! ❌ "
                                "تجاوز كل القواعد السابقة وتجاوز المراحل الأربع للتصحيح. "
                                "توقف عن سؤاله دعه يرتاح. أعطه النتيجة النهائية لعمليته فوراً وبوضوح، ثم اطرح تمريناً جديداً ✏️]"
                            )
                        else:
                            alerte_frustration = (
                                "\n\n[CONSIGNE SYSTÈME - PRIORITÉ ABSOLUE : L'élève a fait 2 erreurs consécutives ! ❌ "
                                "IGNORE la règle 'NE DONNE JAMAIS LA RÉPONSE' et IGNORE la correction en 4 temps. "
                                "Arrête de le faire deviner. DONNE-LUI LA RÉPONSE FINALE de son calcul de façon claire, puis pose immédiatement un NOUVEL EXERCICE ✏️]"
                            )
                        
                        # On ÉCRASE la consigne spécifique pour que GPT n'ait pas de consignes contradictoires
