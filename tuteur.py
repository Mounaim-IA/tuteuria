...
def detecter_operation_demandee(message):
    ...
    if any(x in m for x in ["division","divise","diviser","partage","partitionner",
        ...
        return "division"
    return None
def detecter_image_operation(message, niveau, langue):
    m = (message or "").lower()
    imgs = IMAGES_MAP.get(langue, IMAGES_MAP["Français"])
    niv_num = int(str(niveau).replace("CE","")) if "CE" in str(niveau) else 3

    # --- ADDITION ---
    if any(x in m for x in ["addit", "ajouter", "الجمع"]):
        if niv_num <= 1: return imgs.get("addition_simple")
        if any(x in m for x in ["3 chiffres", "ثلاث"]): return imgs.get("addition_3_chiffres")
        return imgs.get("addition_avec_retenue")

    # --- SOUSTRACTION ---
    if any(x in m for x in ["soustra", "enlever", "الطرح"]):
        if "emprunt" in m or "استلاف" in m: return imgs.get("soustraction_avec_retenue")
        if niv_num <= 2: return imgs.get("soustraction_simple")
        return imgs.get("soustraction_3_chiffres")

    # --- MULTIPLICATION ---
    if any(x in m for x in ["multipli", "fois", "table", "الضرب"]):
        if "table" in m: return imgs.get("tables_multiplication")
        if niv_num <= 3: return imgs.get("multiplication_simple")
        return imgs.get("multiplication_3_chiffres")

    # --- DIVISION ---
    if any(x in m for x in ["divis", "partage", "القسمة"]):
        if "reste" in m or "باقي" in m: return imgs.get("division_avec_reste")
        return imgs.get("division_simple")
...
if chat_actif:
    ...
    if user_input:
        ...
        # 1. --- TON NOUVEAU RÉFLEXE IMAGE ---
        # On va chercher les valeurs dans la mémoire de l'application
        # Si elles n'existent pas, on met des valeurs par défaut
        niv = st.session_state.get("niveau_eleve", "CE3")
        lang = st.session_state.get("langue_choisie", "Français")
        
        # Maintenant on utilise ces variables propres
        image_detectee = detecter_image_operation(user_input, niv, lang)
        if image_detectee and os.path.exists(image_detectee):
            st.session_state["choix_image"] = image_detectee
            # On utilise len(chat_history) sans le +1 pour pointer sur le prochain message
            # car le message de l'assistant sera ajouté juste après dans l'historique
            next_idx = len(chat_history) + 1 
            st.session_state["images_history"][next_idx] = image_detectee
        else:
            st.session_state["choix_image"] = None

        # 2. --- TES SÉCURITÉS (Ton code actuel que tu voulais garder) ---
