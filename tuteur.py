            ...
            if pos >= 0:
                ...
                if match:
                    ...
                    try:
                        ...
                        # ── DIVISION EUCLIDIENNE : jamais de décimal au primaire ──
                        elif '/' in calcul_python and isinstance(resultat, float):
                            _parts_div = calcul_python.split('/')
                            if len(_parts_div) == 2:
                                try:
                                    _a_d = int(float(_parts_div[0].strip()))
                                    _b_d = int(float(_parts_div[1].strip()))
                                    _q_d = _a_d // _b_d
                                    _r_d = _a_d % _b_d
                                    resultat = f"{_q_d} reste {_r_d}" if _r_d != 0 else _q_d
                                except Exception:
                                    pass
    ...
    if parties[0] == 'correct':
        ...
        if langue == "العربية":
            ...
            return (
                ...
                f"3. Pose UNE question socratique pour guider SANS donner {resultat}\n"
...
def detecter_image_operation(message, reponse_tuteur, niveau, langue):
    ...
    if any(mot in msg for mot in ["division", "divise", "partage", "القسمة", "يقسم", "÷"]):
        ...
        # ── Vérifier le reste via les nombres du message ──
        import re as _re_img
        _nums_img = _re_img.findall(r'\d+', msg)
        if len(_nums_img) >= 2:
            try:
                _a_img = int(_nums_img[0])
                _b_img = int(_nums_img[-1])
                if _b_img > 0 and _a_img % _b_img != 0:
...
2. DÉCOMPOSITION VISUELLE VERTICALE : Montre le calcul POSÉ VERTICALEMENT avec les retenues visibles.
   ...
   Toute décomposition verticale avec chiffres alignés quand une image est affichée = VIOLATION GRAVE.
   ✅ Sans image : pose les calculs verticalement avec 🔴 et 🟢.
...
if user_input:
                ...
                if _img_path and _choix_consigne and etape_actuelle == "explication":
                    ...
                    if langue_choisie == "العربية":
                        ...
                        consigne_specifique = (
                            ...
                            f"Ligne 3 : '✏️ À toi ! Combien font {_choix_consigne} ?' — pose directement l'exercice. "
    ...
    # ── Stocker l'image avec le bon index (APRÈS append) ──────
    if _img_path and os.path.exists(_img_path):
        _idx_img = len(st.session_state[session_key]) - 1  # Index du AIMessage
        st.session_state["images_history"][_idx_img] = _img_path
