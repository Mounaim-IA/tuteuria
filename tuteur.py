...
IMAGES_MAP = {
    ...
    "العربية": {
        ...
        "tables_multiplication":       _ar("09_ar_tables_multiplication.png"),
        ...
        "fractions_introduction":    _ar("15_ar_fractions_introduction.png"),
        "fractions_equivalentes":    _ar("16_ar_fractions_equivalentes.png"),
        "simplification_fractions":  _ar("35_ar_simplification_fractions.png"),
        "addition_fractions":        _ar("17_ar_addition_fractions.png"),
        "soustraction_fractions":    _ar("18_ar_soustraction_fractions.png"),
        "comparaison_fractions":     _ar("19_ar_comparaison_fractions.png"),
        "multiplication_fractions":  _ar("33_ar_multiplication_fractions.png"),
        "division_fractions":        _ar("34_ar_division_fractions.png"),
        "fractions_denom_diff":      _ar("32_ar_addition_fractions_denom_diff.png"),
        "fraction_d_un_nombre":      _ar("31_ar_fraction_d_un_nombre.png"),
        "fractions_decimales":       _ar("37_ar_fractions_decimales.png"),
...
def detecter_calcul_direct(message):
    """
    Détecte si l'élève a écrit un calcul direct (ex: "13-7", "27+35", "4×6").
    Retourne (expression, a, op, b) ou None.
    """
    m = (message or "").strip()
    match = re.fullmatch(
        r'(\d+(?:[,\.]\d+)?)\s*([+\-−×*x÷/])\s*(\d+(?:[,\.]\d+)?)',
    ...
    if match:
        ...
        return (m, float(a), op, float(b))
    return None


def get_image_for_calcul(a, op, b, niveau):
    """
    Détermine l'image à afficher pour un calcul direct a op b.
    Analyse les chiffres ET si un emprunt/retenue est nécessaire.
    """
    n    = int(str(niveau).replace("CE","")) if "CE" in str(niveau) else 3
    imgs = IMAGES_MAP.get("Français")  # langue gérée à l'appel

    ia, ib = int(a), int(b)  # valeurs entières pour analyse
    nb_ch_a = len(str(ia))   # nombre de chiffres de a
    nb_ch_b = len(str(ib))   # nombre de chiffres de b

    # ── ADDITION ──
    if op in ['+']:
        # Vérifier s'il y a retenue (unités a + unités b ≥ 10)
        u_a = ia % 10
        u_b = ib % 10
        ...
        if nb_ch_a <= 1 and nb_ch_b <= 1:
            return imgs.get("addition_simple"), "addition simple, 1 chiffre"
        if has_carry:
    ...
    if op in ['-', '−']:
        ...
        u_a = ia % 10
        u_b = ib % 10
        needs_borrow = u_a < u_b  # emprunt nécessaire aux unités

        if nb_ch_a >= 3 and nb_ch_b >= 3:
            return imgs.get("soustraction_3_chiffres"), "soustraction à 3 chiffres"
        if nb_ch_a >= 3 and nb_ch_b == 2:
            if needs_borrow:
                return imgs.get("soustraction_3ch_2ch_emprunt"), "soustraction 3 chiffres − 2 chiffres avec emprunt"
            return imgs.get("soustraction_3_chiffres"), "soustraction à 3 chiffres"
        if nb_ch_a >= 3 and nb_ch_b == 1:
            if needs_borrow:
                return imgs.get("soustraction_3ch_1ch_emprunt"), "soustraction 3 chiffres − 1 chiffre avec emprunt"
            return imgs.get("soustraction_sans_retenue"), "soustraction sans emprunt"
        if nb_ch_a == 2 and nb_ch_b == 2:
            if needs_borrow:
                return imgs.get("soustraction_avec_retenue"), "soustraction 2 chiffres avec emprunt"
            return imgs.get("soustraction_sans_retenue"), "soustraction sans emprunt"
        if nb_ch_a == 2 and nb_ch_b == 1:
            if needs_borrow:
                return imgs.get("soustraction_2ch_1ch_emprunt"), "soustraction 2 chiffres − 1 chiffre avec emprunt"
            return imgs.get("soustraction_simple"), "soustraction simple"
        return imgs.get("soustraction_simple"), "soustraction simple"
    ...
    if op in ['×', '*', 'x']:
        ...
        if nb_ch_a == 2 or nb_ch_b == 2:
            return imgs.get("multiplication_simple"), "multiplication 2 chiffres × 1 chiffre"
    ...
    if op in ['÷', '/']:
        ...
        # Vérifier s'il y a un reste
        if ia % ib != 0:
            return imgs.get("division_avec_reste"), "division avec reste"
        return imgs.get("division_simple"), "division posée, diviseur 1 chiffre"
                    ...
                    if langue_choisie == "العربية":
                        if is_pb:
                            ...
                            consigne_specifique = (
                                ...
                                f"قل 'للمزيد من الفهم، انظر للصورة ! 😊' "
                                f"ثم اشرح بجملتين باستخدام أرقام الصورة فقط (لا تخترع أرقاماً جديدة). "
                                f"اختم بـ '✏️ دورك !' مع : {_exercice_final_ar}]"
                            )
                    else:
                        if is_pb:
                            consigne_specifique = (
                            ...
                            consigne_specifique = (
                                ...
                                f"Dis 'Pour mieux comprendre, regarde bien l'image ! 😊' puis explique en 2 phrases : "
                                f"ligne 1 = les unités multiplient tout, ligne 2 = les dizaines multiplient tout + un point •, puis on additionne les deux lignes. "
                                f"Termine par '✏️ À toi !' avec : {_exercice_final}]"
                            )
                        else:
                            consigne_specifique = (
                                f"[CONSIGNE : Image affichée. Écris 4-5 lignes sans listes ni astérisques. "
                                f"Dis 'Pour mieux comprendre, regarde bien l'image ! 😊' "
                                f"puis explique en 2 phrases en utilisant LES CHIFFRES DE L'IMAGE uniquement. "
                                f"Termine par '✏️ À toi !' avec : {_exercice_final}]"
                            )
