...
if not chat_actif:
    ...
    if btn_clicked:
        ...
        st.session_state["taux_max"] = 0  # Reset barre nouvelle session
        st.rerun()
    ...
    # ============================================================
    # ── Footer TuteurIA → admin même page ───────────────────────
    if "admin_ouvert" not in st.session_state:
    ...
    # ── Footer simple ─────────────────────────────────────────
    st.markdown("""
    <div style="text-align:center;font-family:'Fredoka One',cursive;
                font-size:0.9em;color:#764ba2;margin-top:16px;">
        🧮 TuteurIA | Mounaim 2026 🌟
    </div>
    """, unsafe_allow_html=True)


    if st.session_state["admin_ouvert"]:
        st.markdown("---")
        ...
        if not st.session_state["admin_ok"]:
            ...
            with col_x:
                if st.button("✖️", key="btn_admin_close_pre",
                             use_container_width=True):
                    st.session_state["admin_ouvert"] = False
                    st.rerun()
            ...
            col_titre, col_deco, col_close = st.columns([3, 1, 1])
            with col_titre:
                st.markdown("#### 📊 Tableau de bord")
            with col_deco:
                if st.button("🔒", key="btn_admin_deco",
                             type="secondary", use_container_width=True):
                    st.session_state["admin_ok"] = False
                    st.rerun()
            with col_close:
                if st.button("✖️", key="btn_admin_close",
                             type="secondary", use_container_width=True):
                    st.session_state["admin_ok"]     = False
                    st.session_state["admin_ouvert"] = False
                    st.rerun()
...
# Boutons — même colonnage que les boutons de langue [2,1,0.2,1,2]

# JS style identique aux boutons de langue
_txt_new = "Recommencer" if langue_choisie == "Français" else "من جديد"
_txt_dl  = "Télécharger" if langue_choisie == "Français" else "تحميل"
...
(function() {{
    function styleBtns() {{
        ...
        doc.querySelectorAll('button').forEach(function(btn) {{
            ...
            // STYLE RÉACTIF : S'adapte à la taille de l'écran
            btn.setAttribute('style',
                ...
                /* LE SECRET DU TEXTE : Minimum 0.75rem (mobile), Maximum 1.1rem (PC) */
                ...
                /* LE SECRET DU PADDING : Plus grand sur PC, plus petit sur Mobile */
                'padding:clamp(8px, 1.5vw, 12px) 4px !important;' + 
            ...
            btn.querySelectorAll('p, span, div').forEach(function(el) {{
                el.style.cssText = 'margin:0 !important;padding:0 !important;text-align:center !important;width:100% !important;color:white !important;font-size:inherit !important;overflow:hidden !important;text-overflow:ellipsis !important;white-space:normal !important;';
            }});

            var block = btn.closest('div[data-testid="stHorizontalBlock"]');
            if (block) targetRow = block;
        }});

        if (targetRow) {{
            targetRow.style.setProperty('display', 'grid', 'important');
            
            /* LA SOLUTION POUR LE PC : La taille maximum passe de 140px à 260px ! */
            /* Sur mobile, ils se réduiront automatiquement vers le "0" pour rentrer dans l'écran */
            targetRow.style.setProperty('grid-template-columns', 'minmax(0, 260px) minmax(0, 260px)', 'important'); 
            
            targetRow.style.setProperty('justify-content', 'center', 'important'); 
            targetRow.style.setProperty('gap', '12px', 'important'); /* Un peu d'espace qui rend bien partout */
            targetRow.style.setProperty('width', '100%', 'important');
            targetRow.style.setProperty('max-width', '100vw', 'important');
            targetRow.style.setProperty('box-sizing', 'border-box', 'important');
            targetRow.style.setProperty('padding', '0 10px', 'important');
            
            var cols = targetRow.querySelectorAll('div[data-testid="column"]');
...
with _bn4:
   if st.button(lbl_print, use_container_width=True, key="btn_download_conv"):
        ...
        def _clean(txt):
            """Nettoie le texte : garde uniquement les caractères Latin-1 + courants."""
            import unicodedata
            # Supprimer emojis et symboles hors Latin-1
            txt = _re.sub(r'[\U00010000-\U0010ffff]', '', txt)  # emojis 4 octets
            txt = _re.sub(r'[\u2600-\u27ff]', '', txt)           # symboles divers
            txt = _re.sub(r'[\u2000-\u206f]', '', txt)           # ponctuation spéciale
            txt = _re.sub(r'[\u0600-\u06ff]', '', txt)           # arabe → ignoré en PDF
            txt = txt.replace('**','').replace('*','')
            txt = txt.replace('\u2019',"'").replace('\u2018',"'")
            txt = txt.replace('\u201c','"').replace('\u201d','"')
            txt = txt.replace('\u2013','-').replace('\u2014','-')
            # Garder uniquement les caractères encodables en latin-1
            result = ''
            for c in txt:
                try:
                    c.encode('latin-1')
...
_taux   = round(_bonnes / _total * 100) if _total > 0 else 0

# Option A+ : ne descend jamais
if "taux_max" not in st.session_state:
    st.session_state["taux_max"] = 0
if _taux > st.session_state["taux_max"]:
    st.session_state["taux_max"] = _taux
_taux_aff = st.session_state["taux_max"]
...
_titre = f"🏅 Taux de réussite {_star}" if langue_choisie == "Français" else f"🏅 نسبة النجاح {_star}"
