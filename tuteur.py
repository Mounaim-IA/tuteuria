...
if user_input:
        ...
        if _new_operation and not any(c.isdigit() for c in user_input):
            ...
            if langue_choisie == "العربية":
                ...
                _questions_ch = {
                    ...
                    "division"      : "Pas de problème ! 😊 Tu veux la division simple ou la division posée ?",
                }
    ...
    def _detecter_op_locale(msg):
        m = (msg or "").lower()
        if any(x in m for x in ["addit","ajouter","additionn","الجمع","جمع"]): return "addition"
        if any(x in m for x in ["soustract","soustraire","enlever","retranch","الطرح","طرح"]): return "soustraction"
        if any(x in m for x in ["multipli","fois","table","الضرب","ضرب"]): return "multiplication"
        if any(x in m for x in ["divis","partage","répartir","distribuer","القسمة","قسمة"]): return "division"
        return None
