    ...
    if user_input:
        ...
        if est_changement:
            ...
            if _new_operation and not any(c.isdigit() for c in user_input):
                ...
                _clarif_ch = get_question_clarification(_new_operation, niveau_eleve, langue_choisie)
                if _clarif_ch and _clarif_ch.startswith("HORS_NIV:"):
                    texte_hn = _clarif_ch.replace("HORS_NIV:", "")
                    direction = "rtl" if langue_choisie == "العربية" else "ltr"
                    with st.chat_message("assistant"):
                    ...
                    try:
                        ...
                        pass
                    st.rerun()
                elif _clarif_ch is None:
                    # Un seul sous-type pour ce niveau → pas de clarification, passer directement
                    img_path, consigne = get_image_for_question(_new_operation, niveau_eleve)
                    if consigne: st.session_state["choix_consigne"] = consigne
                    if img_path and os.path.exists(img_path): st.session_state["choix_image"] = img_path
                    st.session_state["hors_niveau_operation"] = _new_operation
                    st.session_state["derniere_operation"] = _new_operation
                    st.session_state[etape_key] = "explication"
                    # continue vers GPT normalement
                else:
                    pass  # _clarif_ch est défini, on continue
                direction = "rtl" if langue_choisie == "العربية" else "ltr"
                with st.chat_message("assistant"):
        ...
        if (_operation_universelle
            ...
            st.session_state[etape_key] = "clarification"
            _clarif_univ = get_question_clarification(_operation_universelle, niveau_eleve, langue_choisie)
            if _clarif_univ and _clarif_univ.startswith("HORS_NIV:"):
                    texte_hn = _clarif_ch.replace("HORS_NIV:", "")  # réutilisé ci-dessous
                    direction = "rtl" if langue_choisie == "العربية" else "ltr"
                    with st.chat_message("assistant"):
                        st.markdown(f'<div dir="{direction}">{texte_hn}</div>', unsafe_allow_html=True)
                    st.session_state[session_key].append(HumanMessage(content=user_input))
                    ...
                    try:
                        ...
                        pass
                    st.rerun()
            elif _clarif_univ is None:
                img_path_u, consigne_u = get_image_for_question(_operation_universelle, niveau_eleve)
                if consigne_u: st.session_state["choix_consigne"] = consigne_u
                if img_path_u and os.path.exists(img_path_u): st.session_state["choix_image"] = img_path_u
                st.session_state["hors_niveau_operation"] = _operation_universelle
                st.session_state["derniere_operation"] = _operation_universelle
                st.session_state[etape_key] = "explication"
                etape_actuelle = "explication"
            else:
                pass  # _clarif_univ défini, continue
            direction = "rtl" if langue_choisie == "العربية" else "ltr"
            with st.chat_message("assistant"):
        ...
        if etape_actuelle == "amorce":
            ...
            if calcul:
                ...
                if _pas_de_chiffres:
                    ...
                    _clarif_msg = get_question_clarification(operation, niveau_eleve, langue_choisie)
                    if _clarif_msg and _clarif_msg.startswith("HORS_NIV:"):
                        texte_hn = _clarif_msg.replace("HORS_NIV:", "")
                        direction = "rtl" if langue_choisie == "العربية" else "ltr"
                        with st.chat_message("assistant"):
                            st.markdown(f'<div dir="{direction}">{texte_hn}</div>', unsafe_allow_html=True)
                        st.session_state[session_key].append(HumanMessage(content=user_input))
                        ...
                        try:
                            ...
                            pass
                        st.rerun()
                    elif _clarif_msg is None:
                        img_path_b, consigne_b = get_image_for_question(operation, niveau_eleve)
                        if consigne_b: st.session_state["choix_consigne"] = consigne_b
                        if img_path_b and os.path.exists(img_path_b): st.session_state["choix_image"] = img_path_b
                        st.session_state["hors_niveau_operation"] = operation
                        st.session_state["derniere_operation"] = operation
                        st.session_state[etape_key] = "explication"
                        etape_actuelle = "explication"
                        _clarif_msg = ""  # pas de message de clarification
                ...
                # ── CAS 3 : Problème énoncé avec chiffres → image précise via calcul extrait ──
                    ...
                    if not detecter_probleme_enonce(user_input):
                        st.session_state["hors_niveau_operation"] = operation
                        st.session_state[etape_key] = "clarification"
                        _clarif_c3 = get_question_clarification(operation, niveau_eleve, langue_choisie)
                        if _clarif_c3 and _clarif_c3.startswith("HORS_NIV:"):
                            texte_hn = _clarif_c3.replace("HORS_NIV:", "")
                            direction = "rtl" if langue_choisie == "العربية" else "ltr"
                            with st.chat_message("assistant"):
                                st.markdown(f'<div dir="{direction}">{texte_hn}</div>', unsafe_allow_html=True)
                            st.session_state[session_key].append(HumanMessage(content=user_input))
                            ...
                            try:
                                ...
                                pass
                            st.rerun()
                        elif _clarif_c3 is None:
                            img_path_c3, consigne_c3 = get_image_for_question(operation, niveau_eleve)
                            if consigne_c3: st.session_state["choix_consigne"] = consigne_c3
                            if img_path_c3 and os.path.exists(img_path_c3): st.session_state["choix_image"] = img_path_c3
                            st.session_state["hors_niveau_operation"] = operation
                            st.session_state["derniere_operation"] = operation
                            st.session_state[etape_key] = "explication"
                            etape_actuelle = "explication"
