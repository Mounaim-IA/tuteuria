...
→ Si INCORRECT — SÉQUENCE OBLIGATOIRE EN 4 TEMPS :
   ...
   Ensuite, RÉSOUS ENTIÈREMENT ce nouveau calcul inventé, étape par étape, en donnant son résultat final.
   Exemple si l'élève bloquait sur 45-28 : "Pour 53 - 26, j'emprunte une dizaine car 3 est plus petit que 6. 13 - 6 = 7, et 4 - 2 = 2. Ça fait 27 !"
   
   ⚠️ RÈGLE ABSOLUE DU TEMPS 2 :
   ✅ Tu DOIS donner le résultat final du calcul SIMILAIRE que tu as inventé.
...
if user_input:
    ...
    if "_debug" in st.session_state:
        st.caption(f"DEBUG: {st.session_state['_debug']}")

    if not _skip_gpt:
        with st.chat_message("assistant"):
            st.session_state["_debug"] = {
                "etape": st.session_state[etape_key],
                "choix_image": str(st.session_state.get("choix_image")),
                "exercice": str(extraire_exercice(chat_history))
        }
            st.caption(f"DEBUG — étape: {st.session_state[etape_key]} | choix_image: {st.session_state.get('choix_image')} | choix_consigne: {st.session_state.get('choix_consigne')}")
            st.caption(f"DEBUG extraire_exercice: {extraire_exercice(chat_history)}")
            with st.spinner(t["thinking"]):
                try:
                    ...
                    elif _verdict_pre and isinstance(_verdict_pre, str) and _verdict_pre.startswith("presque"):
                        pass
                    ...
                    if st.session_state["erreurs_consec"] >= 2:
                        ...
                        # 3. On remet le compteur à zéro pour le prochain calcul
                        st.session_state["erreurs_consec"] = 0
                    # ------------------------------------------------
                    _consigne_sys = ""
