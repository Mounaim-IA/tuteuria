...
(function() {{
    function styleBtns() {{
        ...
        doc.querySelectorAll('button').forEach(function(btn) {{
            ...
            btn.querySelectorAll('p, span, div').forEach(function(el) {{
                el.style.cssText = 'margin:0 !important;padding:0 !important;' +
                    'text-align:center !important;width:100% !important;color:white !important;font-size:inherit !important;';
            }});

            var block = btn.closest('div[data-testid="stHorizontalBlock"]');
            if (block) targetBlock = block;
        }});

        // La magie pour partager l'espace parfaitement sur Mobile
        if (targetBlock) {{
            targetBlock.style.setProperty('display', 'flex', 'important');
            targetBlock.style.setProperty('flex-direction', 'row', 'important');
            targetBlock.style.setProperty('flex-wrap', 'nowrap', 'important');
            targetBlock.style.setProperty('width', '100%', 'important');
            targetBlock.style.setProperty('max-width', '100vw', 'important');
            targetBlock.style.setProperty('gap', '10px', 'important');
            
            var cols = targetBlock.querySelectorAll('div[data-testid="column"]');
            
            // S'il y a les 5 colonnes (vides + boutons)
            if (cols.length === 5) {{
                cols[0].style.setProperty('display', 'none', 'important'); // Cache vide gauche
                cols[2].style.setProperty('display', 'none', 'important'); // Cache vide centre
                cols[4].style.setProperty('display', 'none', 'important'); // Cache vide droite
                
                // Force exactement 50% pour Recommencer
                cols[1].style.setProperty('width', '50%', 'important');
                cols[1].style.setProperty('min-width', '0', 'important');
                cols[1].style.setProperty('flex', '1 1 0%', 'important');
                
                // Force exactement 50% pour Télécharger
                cols[3].style.setProperty('width', '50%', 'important');
                cols[3].style.setProperty('min-width', '0', 'important');
                cols[3].style.setProperty('flex', '1 1 0%', 'important');
            }} 
            // Sécurité : Si vous n'avez que 2 colonnes
    ...
    setTimeout(styleBtns, 100);
    setTimeout(styleBtns, 500);
    setTimeout(styleBtns, 1200);
    new MutationObserver(styleBtns).observe(
        window.parent.document.body, {{childList:true, subtree:true}}
    );
}})();
</script>
