    ...
    with st.chat_message(role):
        if role == "assistant":
            ...
            if _img_stored and os.path.exists(_img_stored):
                st.image(_img_stored, width=300)
...
if user_input:
    ...
    # ── DEBUG TEMPORAIRE ──
    st.toast(f"🏷️ etape={etape_actuelle}", icon="🏷️")
    _skip_gpt = False
    if etape_actuelle == "amorce":
        operation = detecter_operation_demandee(user_input)
        calcul = detecter_calcul_direct(user_input)
        # ── DEBUG TEMPORAIRE ──
        _gen = est_question_generique(user_input) if operation else "N/A"
        st.toast(f"🔍 op={operation}, calc={calcul}, gen={_gen}", icon="🔍")
        ...
        with st.chat_message("assistant"):
            with st.spinner(t["thinking"]):
                try:
                    ...
                    if _img_path:
                        st.image(_img_path, width=300)  # Utilise la même taille qu'au-dessus
