...
def post_traitement(reply, user_input, historique, langue):
    ...
    if gpt_faux:
        ...
        if langue == "العربية":
            return f"👏 محاولة جيدة ! 😊 لكن الجواب ليس صحيحاً تماماً.\nلنحاول معاً : فكر جيداً وحاول مرة أخرى ! 🤔"
        else:
            return f"👏 Bonne tentative ! 😊 Mais ce n'est pas tout à fait ça.\nEssayons ensemble : réfléchis bien et essaie encore ! 🤔"
...
EXPLICATIONS_IMAGES = {
    "addition_simple": {
        "Français": "Regarde l'image ! 😊 Elle montre 3 billes bleues + 2 billes bleues = 5 billes vertes. C'est ça l'addition : on met tout ensemble !",
        "العربية": "انظر للصورة ! 😊 تُظهر 3 كرات زرقاء + 2 كرات زرقاء = 5 كرات خضراء. هذا هو الجمع : نضع الكل معاً !"
    },
    ...
    "soustraction_simple": {
        "Français": "Regarde l'image ! 😊 Elle montre 5 billes bleues, on enlève 2 billes (barrées en rouge), il reste 3 billes vertes. C'est ça la soustraction : on enlève !",
        "العربية": "انظر للصورة ! 😊 تُظهر 5 كرات زرقاء، نزيل 2 كرات (مشطوبة بالأحمر)، يبقى 3 كرات خضراء. هذا هو الطرح : ننزع !"
    },
    ...
    "division_simple": {
        "Français": "Regarde l'image ! 😊 La division posée : on divise étape par étape. D'abord on cherche combien de fois le diviseur rentre dans le premier chiffre, on soustrait (en rouge), puis on descend le chiffre suivant (flèche rouge). Le quotient est en vert à droite.",
    ...
    "multiples_diviseurs": {
        "Français": "Regarde l'image ! 😊 Un multiple c'est le résultat d'une multiplication (multiples de 3 : 3, 6, 9, 12...). Un diviseur divise un nombre sans reste (diviseurs de 12 : 1, 2, 3, 4, 6, 12). Les astuces en couleur t'aident à les reconnaître !",
        "العربية": "انظر للصورة ! 😊 المضاعف هو نتيجة الضرب (مضاعفات 3 : 3، 6، 9، 12...). القاسم يقسم العدد بدون باقٍ (قواسم 12 : 1، 2، 3، 4، 6، 12)."
    },
}
...
def detecter_calcul_direct(message):
    ...
    if not match:
        # Fallback: chercher un calcul embarqué dans du texte
        match = re.search(
            r'(\d+(?:[,\.]\d+)?)\s*([+\-−×*x÷/])\s*(\d+(?:[,\.]\d+)?)', m
        )
    if not match:
...
→ Si INCORRECT — SÉQUENCE OBLIGATOIRE EN 4 TEMPS :
   ...
   Explique POURQUOI la réponse est fausse en s'appuyant sur la méthode montrée dans l'image.
   Commence TOUJOURS par : "Rappelle-toi l'image qu'on a vue ! 😊 On va faire pareil avec tes chiffres :"
   
   Pour un calcul SIMPLE (1 chiffre) :
   "Essayons ensemble : mets 7 dans ta tête 🧠,
   lève 5 doigts 🤚 et compte : 8, 9, 10, 11, 12. On arrive à 12 ! 😊"
   
   Pour les TABLES DE MULTIPLICATION :
   "On cherche 7 × 8. C'est comme additionner 8 sept fois :
   8, 16, 24, 32, 40, 48, 56. Donc 7 × 8 = 56 ! 😊"
   
   Pour un calcul POSÉ (2-3 chiffres) — MÉTHODE OBLIGATOIRE :
   "Rappelle-toi l'image ! 😊 On pose le calcul : les unités sous les unités, les dizaines sous les dizaines."
   Puis explique colonne par colonne avec les VRAIS chiffres de l'exercice, comme l'image le montre :
   
   - Addition SANS retenue : "Les unités : 4 + 3 = 7. Les dizaines : 2 + 1 = 3. Résultat : 37."
   - Addition AVEC retenue : "Les unités : 7 + 5 = 12. Comme dans l'image, le chiffre dépasse 9, on écrit 2 et on retient 1 (le rond rouge 🔴). Les dizaines : 2 + 3 + 1 (retenue) = 6. Résultat : 62."
   - Addition 3 CHIFFRES : Explique TOUJOURS les trois colonnes : unités, dizaines ET centaines. "Les unités : 5 + 7 = 12, on écrit 2 et on retient 1. Les dizaines : 4 + 8 + 1 (retenue) = 13, on écrit 3 et on retient 1. Les centaines : 3 + 2 + 1 (retenue). Combien ça fait ? Essaie ! 🤔"
   - Soustraction SANS emprunt : "Les unités : 8 - 3 = 5. Les dizaines : 4 - 2 = 2. Résultat : 25."
   - Soustraction AVEC emprunt : "Les unités : 3 est plus petit que 8. Comme dans l'image, quand le chiffre du haut est plus petit, on emprunte 1 dizaine (le rond rouge 🔴). 13 - 8 = 5. Les dizaines : on avait 4, on a prêté 1, il reste 3. 3 - 2 = 1. Résultat : 15."
   - Soustraction 3 CHIFFRES : Explique TOUJOURS les trois colonnes : unités, dizaines ET centaines. "Les unités : 5 est plus petit que 8, on emprunte. Combien font 15 - 8 ? Les dizaines : 3 est plus petit que 6, on emprunte aux centaines. Combien font 13 - 6 ? Les centaines : on avait 3, on a prêté 1. Combien reste-t-il ? Essaie ! 🤔"
   - Soustraction DOUBLE EMPRUNT : "Les unités : 0 est plus petit que 7, on emprunte aux dizaines. Mais les dizaines valent 0 aussi, donc comme l'image le montre, on emprunte d'abord aux centaines. Les centaines perdent 1, les dizaines deviennent 9 (car on re-prête 1 aux unités), les unités deviennent 10."
   - Multiplication SIMPLE (1×2 ch) : "Les unités : 4 × 3 = 12. Comme dans l'image, on écrit 2 et on retient 1 (le rond rouge 🔴). Les dizaines : 2 × 3 = 6 + 1 (retenue) = 7. Résultat : 72."
   - Multiplication 3 CHIFFRES × 1 (ex: 245 × 7) : Explique TOUJOURS les trois colonnes. "Les unités : 5 × 7, combien ça fait ? On écrit le chiffre des unités et on retient. Les dizaines : 4 × 7 + la retenue. Les centaines : 2 × 7 + la retenue. Essaie ! 🤔"
   - Multiplication 2×2 CHIFFRES : "Comme dans l'image, on fait deux lignes. Ligne 1 : on multiplie tout par les unités du deuxième nombre. Ligne 2 : on multiplie tout par les dizaines et on décale d'un cran. Puis on additionne les deux lignes."
   - Multiplication 3×2 CHIFFRES (ex: 245 × 36) : "Comme dans l'image, on fait deux lignes. Ligne 1 : on multiplie 245 par les unités du deuxième nombre (6), colonne par colonne. Ligne 2 : on multiplie 245 par les dizaines (3) et on décale d'un cran. Combien trouves-tu pour chaque ligne ? Puis additionne les deux ! Essaie ! 🤔"
   - Division SIMPLE : "On cherche combien de fois 4 rentre dans 84. 4 × 2 = 8 pour les dizaines, 4 × 1 = 4 pour les unités. Résultat : 21."
   - Division AVEC RESTE : "On cherche combien de fois [b] rentre dans [a]. Compte avec moi : [b]×1=[?], [b]×2=[?], [b]×3=[?]... On s'arrête quand le résultat dépasse [a]. Essaie ! 🤔"
   - Division DIVISEUR 2 CHIFFRES : "On cherche combien de fois 12 rentre dans 156. 12 × 1 = 12 (pour 15), reste 3. On descend le 6 : 36 ÷ 12 = 3. Résultat : 13."
   - Division 3 CHIFFRES (ex: 156 ÷ 12) : Explique étape par étape. "On commence par les chiffres de gauche : combien de fois 12 rentre dans 15 ? Ensuite on descend le chiffre suivant. Combien de fois 12 rentre dans le nouveau nombre ? Essaie ! 🤔"
   
   ❌ INTERDIT : décomposition verticale avec des colonnes de chiffres alignés.
   ✅ OBLIGATOIRE : tout expliquer en phrases horizontales comme ci-dessus.
   ✅ Utilise TOUJOURS les vrais chiffres de l'exercice, pas ceux des exemples ci-dessus.
   ✅ Fais TOUJOURS le lien avec l'image : "Comme dans l'image...", "Rappelle-toi les ronds rouges 🔴..."
   ⚠️ RÈGLE ABSOLUE DU TEMPS 2 :
   Tu peux expliquer la MÉTHODE (comment poser, comment emprunter, comment compter).
   Tu ne dois JAMAIS donner le RÉSULTAT FINAL dans l'explication.
   ❌ INTERDIT : "6 fois 3 fait 18, il reste 5" (c'est la réponse !)
   ❌ INTERDIT : "Les unités : 17 - 9 = 8. Les dizaines : 2 - 2 = 0." (c'est la réponse !)
   ✅ CORRECT : "Les unités : 7 est plus petit que 9, donc on emprunte. Combien font 17 - 9 ? Essaie ! 🤔"
   ✅ CORRECT : "On cherche combien de fois 6 rentre dans 23. Compte : 6×1=6, 6×2=12, 6×3=? Continue ! 🤔"
   L'élève doit TROUVER le résultat, pas le LIRE dans ton explication.

   TEMPS 3 — GUIDAGE VERS LA RÉPONSE :
   ❌ INTERDIT : poser une sous-question différente de l'exercice (comme "combien font 14-8 ?").
   ❌ INTERDIT : poser un calcul intermédiaire. Le système ne reconnaît QUE l'exercice original.
   ✅ OBLIGATOIRE : redemander le MÊME exercice original.
   Exemple : "Maintenant que tu as compris la méthode, essaie encore ! 😊
   ✏️ Combien font 54 - 28 ? 🤔"
   → ATTENDS que l'élève réponde.
...
if user_input:
    ...
    est_changement = (
        ...
        and _new_operation is not None
        and (
            any(m in user_input.lower() for m in mots_changement)
            or etape_actuelle == "felicitations"
            or not any(c.isdigit() for c in user_input)
        )
    )
    if est_changement:
        st.session_state[etape_key]       = "amorce"
            ...
            with st.spinner(t["thinking"]):
                try:
                    ...
                    if _img_path and _choix_consigne and etape_actuelle == "explication":
                        ...
                        _img_key = None
                        for key, path in IMAGES_MAP.get("Français", {}).items():
                            if path == _img_path:
                                _img_key = key
                                break
                        if _img_key is None:
                            for key, path in IMAGES_MAP.get("العربية", {}).items():
                                if path == _img_path:
                                    _img_key = key
                                    break
                        _img_desc = EXPLICATIONS_IMAGES.get(_img_key or "", {}).get(
                            langue_choisie if langue_choisie in ["Français", "العربية"] else "Français", ""
                        )
                        ...
                        if langue_choisie == "العربية":
                            ...
                            if is_pb:
                                ...
                                consigne_specifique = (
                                    ...
                                    f"Ligne 2 : Référence à un élément VISUEL spécifique de l'image (rond rouge 🔴, flèche, colonne D/U...) "
                        ...
                        if langue_choisie == "العربية":
                            if is_pb:
                                ...
                                consigne_specifique = (
                                    f"[تعليمات : {_img_desc} "
                                    f"❌ ممنوع : إعادة شرح الطريقة أو وصف خطوات الحساب. "
                                    f"✅ الشكل في 3 أسطر كحد أقصى : "
                                    f"السطر 1 : 'لحل هذا الحساب، نستخدم [اسم العملية] !' "
                                    f"السطر 2 : الإشارة لعنصر بصري محدد في الصورة (الدائرة الحمراء 🔴، السهم، عمود ع/آ...) "
                                    f"السطر 3 : '✏️ دورك ! كم يساوي [أ] [عملية] [ب] ؟' مع : {_exercice_final_ar}. "
                                    f"لا تعطِ الجواب أبداً. انتظر التلميذ.]"
                                )
                        else:
