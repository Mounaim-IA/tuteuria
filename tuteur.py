...
if chat_actif:
    ...
    if user_input:
        ...
        image_detectee = detecter_image_operation(user_input, niveau_eleve, langue_choisie)
        if image_detectee and os.path.exists(image_detectee):
            st.session_state["choix_image"] = image_detectee
            if st.session_state["choix_image"]:
                # On sauvegarde l'index du prochain message pour associer l'image
                next_idx = len(chat_history) + 1 
                st.session_state["images_history"][next_idx] = st.session_state["choix_image"]
        else:
            st.session_state["choix_image"] = None
