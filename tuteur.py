...
if langue_choisie == "Français":
    st.markdown('<div class="progress-badge">🧮 Mathématiques — Cycle Primaire (CE1 → CE6)</div>', unsafe_allow_html=True)
else:
    st.markdown('<div class="progress-badge" dir="rtl">🧮 الرياضيات — السلك الابتدائي (المستوى الأول → السادس)</div>', unsafe_allow_html=True)
...
🔴 Opérations mixtes (3+2-1) : ACCEPTÉES et traitées normalement ✅
   → Ne bloque JAMAIS une opération mathématiquement correcte.
...
# Amorce → boutons Nouvelle conversation + Imprimer
if etape_actuelle == "amorce":
    lbl_new   = "🔄 Nouvelle conversation" if langue_choisie == "Français" else "🔄 محادثة جديدة"
    lbl_print = "🖨️ Imprimer" if langue_choisie == "Français" else "🖨️ طباعة"
    col_a, col_b = st.columns(2)
    with col_a:
        if st.button(lbl_new, use_container_width=True):
    ...
    with col_b:
        ...
        <button onclick="window.print()"
            style="width:100%;background:linear-gradient(135deg,#FF6B6B,#FFE66D);
                   color:#333;border:none;padding:10px;border-radius:15px;
                   font-family:'Fredoka One',cursive;font-size:1.05em;
                   font-weight:bold;cursor:pointer;">{lbl_print}</button>
        """, unsafe_allow_html=True)
elif etape_label:
...
# ============================================================
# 15. SIDEBAR — Nouvelle conversation
# ============================================================
with st.sidebar:
    st.markdown("### 🧮 Tuteur Maths")
    st.markdown("---")

    btn_label = "🔄 Nouvelle conversation" if langue_choisie == "Français" else "🔄 محادثة جديدة"
...
# ============================================================
# BOUTON IMPRIMER — Simple, UN clic, adapté aux enfants
# ============================================================
if len(chat_history) > 1:
    lbl_print = "🖨️ Imprimer la leçon" if langue_choisie == "Français" else "🖨️ طباعة الدرس"
    # Bouton qui déclenche window.print() directement via JS
    st.markdown(f"""
    <div class="no-print" style="text-align:center; margin: 8px 0;">
        <button onclick="window.print()"
            style="background:linear-gradient(135deg,#FF6B6B,#FFE66D);
                   color:#333; border:none; padding:12px 28px;
                   border-radius:50px; font-size:1.05em; font-weight:bold;
