...
if not chat_actif:
    ...
    # ── Barre taux de réussite — Option A+ ────────────────────
    sc = st.session_state[score_key]
    taux_actuel = round(sc["bonnes"] / sc["total"] * 100) if sc["total"] > 0 else 0

    # Option A+ : la barre ne descend jamais
    if "taux_max" not in st.session_state:
        st.session_state["taux_max"] = 0
    if taux_actuel > st.session_state["taux_max"]:
        st.session_state["taux_max"] = taux_actuel
    taux_affiche = st.session_state["taux_max"]

    # Couleur selon le taux (progression logique chaud → froid)
    if taux_affiche >= 80:
        bar_color = "linear-gradient(90deg, #4ECDC4, #00b894)"
        star = "🌟"
    elif taux_affiche >= 50:
        bar_color = "linear-gradient(90deg, #FF9A3C, #FFD93D)"
        star = "⭐"
    else:
        bar_color = "linear-gradient(90deg, #FF6B6B, #FF9A3C)"
        star = "💪"

    # Titre bilingue
    if langue_choisie == "Français":
        titre_barre = "🏅 Taux de réussite"
        lbl_score_b = f"{sc['bonnes']}/{sc['total']} bonnes réponses"
    else:
        titre_barre = "🏅 نسبة النجاح"
        lbl_score_b = f"{sc['bonnes']}/{sc['total']} إجابات صحيحة"
    ...
    with col_bar:
        ...
        <div style="
            ...
            font-family: 'Fredoka One', cursive;
        ">
            <div style="font-size:13px; color:#0F6E56; font-weight:700;
                        margin-bottom:6px; letter-spacing:0.3px;">
                {titre_barre} {star}
            </div>
            <div style="background:#f0f0f0; border-radius:20px;
                        height:16px; overflow:hidden; margin-bottom:5px;
            ...
            <div style="font-size:18px; color:#0F6E56; font-weight:800;">
                {taux_affiche}%
            </div>
...
with col_n:
    if st.button(lbl_new, use_container_width=True, key="btn_new"):
        ...
        st.rerun()
with col_p:
   if st.button(lbl_print, use_container_width=False, key="btn_download_conv"):
        import datetime as _dt
