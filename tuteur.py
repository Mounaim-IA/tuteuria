...
def get_image_for_calcul(a, op, b, niveau):
    ...
    if op in ['+']:
        ...
        if is_decimal:
            return imgs.get("addition_decimaux"),    "addition décimaux, exemple image : 12,5 + 3,45 = 15,95"
        ...
        if has_carry:
            return imgs.get("addition_avec_retenue"),"addition avec retenue, exemple image : 47 + 35 = 82"
    ...
    if op in ['-', '−']:
        ...
        if nb_ch_a >= 3 and nb_ch_b == 1:
            ...
            return imgs.get("soustraction_sans_retenue"),     "soustraction sans emprunt, exemple image : 45 − 23 = 22"
        if nb_ch_a == 2 and nb_ch_b == 2:
            if needs_borrow:
                return imgs.get("soustraction_avec_retenue"), "soustraction 2ch avec emprunt, exemple image : 52 − 27 = 25"
            return imgs.get("soustraction_sans_retenue"),     "soustraction sans emprunt, exemple image : 45 − 23 = 22"
    ...
    if op in ['÷', '/']:
        ...
        return     imgs.get("division_simple"),          "division simple, exemple image : 45 ÷ 5 = 9"
...
def extraire_calcul_depuis_probleme(message, operation):
    """
    Extrait les nombres principaux d'un problème énoncé et construit
    l'expression mathématique correspondante.
    Ex: "59 cahiers pour 8 professeurs" + "division" → "59 ÷ 8"
    """
    import re
...
if user_input:
    ...
    if etape_actuelle == "amorce":
        ...
        if calcul and not operation:
            ...
            if img_path and langue_choisie == "العربية":
                ...
                for key, path in IMAGES_MAP["Français"].items():
                    if path == img_path:
                        ...
                        break
            # Stocker la consigne TOUJOURS (même si l'image n'existe pas)
            # → garantit que GPT reçoit le bon type d'exercice
            if consigne:
            ...
            # ── CAS 3 : Question spécifique → image selon l'opération GPT ──
            else:
                # Utiliser l'opération déjà détectée par GPT (pas re-scanner le texte)
                # On injecte le nom de l'opération dans le message pour guider la détection
                    ...
                    if langue_choisie == "العربية":
                        if is_pb:
                            ...
                            consigne_specifique = (
                                ...
                                f"ثم اشرح بجملتين باستخدام الأرقام التالية فقط : {_choix_consigne}. "
                                f"لا تستخدم أرقاماً أخرى. لا تحسب ولا تعطِ إجابة التمرين. "
                        ...
                        if is_pb:
                            ...
                            consigne_specifique = (
                                f"[CONSIGNE : Ceci est une opération mathématique valide. "
                                f"Image affichée. Écris 4-5 lignes sans listes ni astérisques. "
                                f"Dis 'Pour mieux comprendre, regarde bien l'image ! 😊' "
                                f"puis explique en 2 phrases en utilisant UNIQUEMENT les chiffres indiqués ici : {_choix_consigne}. "
                                f"N'utilise PAS d'autres chiffres dans l'explication. "
                                f"NE CALCULE PAS et NE DONNE PAS la réponse de l'exercice. "
                                f"Termine par '✏️ À toi !' avec : {_exercice_final}]"
                            )
