...
if not chat_actif:
    ...
    # Boutons HTML purs — taille fixe garantie
    lbl_n = "🔄 Recommencer" if langue_choisie == "Français" else "🔄 من جديد"
    lbl_d = "⬇️ Télécharger" if langue_choisie == "Français" else "⬇️ تحميل"

    _, _bn2, _bn4, _ = st.columns([4, 1, 1, 4])
    with _bn2:
        if st.button(lbl_n, use_container_width=True, key="btn_new"):
    ...
    with _bn4:
        if st.button(lbl_d, use_container_width=True, key="btn_download_conv"):
            import datetime as _dt
...
# Option A+ : ne descend jamais SAUF si les deux valeurs sont incohérentes
if "taux_max" not in st.session_state:
    st.session_state["taux_max"] = 0
# Recalcul forcé si incohérence fraction/pourcentage
_taux_recalc = round(_bonnes / _total * 100) if _total > 0 else 0
if _taux_recalc > st.session_state["taux_max"]:
    st.session_state["taux_max"] = _taux_recalc
# Afficher le vrai taux recalculé (pas le max) pour cohérence avec la fraction
_taux_aff = _taux_recalc
_taux_aff = _taux_recalc
