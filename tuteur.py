...
(function() {{
    function styleBtns() {{
        var doc = window.parent.document;
        var targetBlock = null; // Va stocker la ligne contenant les boutons
        ...
        doc.querySelectorAll('button').forEach(function(btn) {{
            ...
            // 1. Style des boutons (ajusté pour mobile)
            btn.setAttribute('style',
                ...
                'border-radius:50px !important;' +
                'font-family:Fredoka One,cursive !important;' +
                'font-size:0.8rem !important;' + /* Taille légèrement réduite pour mobile */
                'box-shadow:none !important;' +
                'background-image:none !important;' +
                'display:flex !important;' +
                'align-items:center !important;' +
            ...
            // 2. Trouver la "ligne" parente (stHorizontalBlock)
            var block = btn.closest('div[data-testid="stHorizontalBlock"]');
            if (block) targetBlock = block;
        }});

        // 3. Forcer la ligne à rester horizontale sur Mobile
        if (targetBlock) {{
            targetBlock.style.setProperty('flex-direction', 'row', 'important');
            targetBlock.style.setProperty('flex-wrap', 'nowrap', 'important');
            targetBlock.style.setProperty('align-items', 'center', 'important');
            
            // 4. Forcer la taille des 5 colonnes pour éviter le retour à la ligne
            var cols = targetBlock.querySelectorAll('div[data-testid="column"]');
            if (cols.length === 5) {{ 
                cols[0].style.setProperty('width', '2%', 'important');  // Marge gauche
                cols[1].style.setProperty('width', '40%', 'important'); // Bouton Recommencer
                cols[2].style.setProperty('width', '2%', 'important');  // Espace central
                cols[3].style.setProperty('width', '54%', 'important'); // Bouton Télécharger (un peu plus large car le texte est plus long)
                cols[4].style.setProperty('width', '2%', 'important');  // Marge droite
...
_bn1, _bn2, _bn3, _bn4, _bn5 = st.columns([1, 2, 0.2, 2, 1])
with _bn2:
