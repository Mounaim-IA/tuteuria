...
def db_maj_session(session_id, bonnes, total, nb_messages, etape_finale):
...
<style>
    ...
    .start-form {
        background: linear-gradient(135deg, #fff9f0, #f0fffe);
...
if not chat_actif:
    ...
    titre_form = "👋 Bienvenue ! Dis-moi qui tu es pour commencer" if langue_choisie == "Français" else "👋 مرحباً ! أخبرني من أنت لنبدأ"
    lbl_prenom = "👦 Ton prénom" if langue_choisie == "Français" else "👦 اسمك"
    lbl_niveau = "📚 Ton niveau" if langue_choisie == "Français" else "📚 مستواك"
    lbl_start  = "🚀 Commencer la leçon !" if langue_choisie == "Français" else "🚀 لنبدأ الدرس !"

    st.markdown(f'<div class="start-form" dir="{direction}"><h3>{titre_form}</h3></div>', unsafe_allow_html=True)
    ...
    if st.button(lbl_start, use_container_width=True,
        ...
        st.rerun()
    # ── Footer toujours visible avant st.stop() ─────────────

# ============================================================
# 17. FOOTER — Bouton TuteurIA ouvre admin dans la page
# ============================================================
...
with col_f2:
    lbl_footer = "🧮 TuteurIA | Mounaim 2026 🌟"
    if st.button(lbl_footer, key="btn_footer_admin", use_container_width=True):
        st.session_state["admin_ouvert"] = not st.session_state.get("admin_ouvert", False)
        if not st.session_state["admin_ouvert"]:
            st.session_state["admin_ok"] = False
        st.rerun()

# Style discret pour le bouton footer
st.markdown("""
<style>
...
# ── Espace Admin — s'ouvre dans la page ─────────────────────
if st.session_state.get("admin_ouvert", False):
    st.markdown("---")
    ...
    if not st.session_state["admin_ok"]:
        ...
        with col_deco:
            if st.button("🚪 Déco.", key="btn_admin_deco"):
                st.session_state["admin_ok"] = False
                st.rerun()
