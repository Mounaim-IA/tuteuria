    ...
    with st.chat_message("assistant"):
        with st.spinner(t["thinking"]):
            try:
                ...
                # ── 2. Pré-détection image AVANT GPT ──────
                # On détecte l'image MAINTENANT pour que GPT sache
                # qu'une image est affichée et ne répète pas l'explication
                etape_actuelle = st.session_state[etape_key]
                _img_path = None
                if etape_actuelle in ("amorce", "explication"):
                    _img_path = detecter_image_operation(
                        user_input, "",
                        eleve_info.get("niveau", ""), langue_choisie
                    )
                ...
                # Si image trouvée, la consigne "explication" dit à GPT
                # de ne PAS répéter ce que l'image montre
                if _img_path and etape_actuelle == "amorce":
                    # Forcer l'étape explication puisqu'on a trouvé une image
                    message_final = injecter_consigne_etape(
                        message_avec_verdict, "explication", langue_choisie
                    )
                else:
                    message_final = injecter_consigne_etape(
