...
<style>
    ...
    [data-testid="stDownloadButton"] > button {
        ...
        font-size: 1.2rem !important; /* Légèrement plus grand pour la lisibilité */
        font-weight: 600 !important;   /* Un peu plus gras pour correspondre au titre */
        border-radius: 15px !important;
        padding: 12px 30px !important;
        background: linear-gradient(135deg, #FF6B6B, #FFE66D) !important;
        color: #ffffff !important;     /* Changé en blanc pour mieux contraster avec le dégradé */
        border: none !important;
        text-shadow: 1px 1px 2px rgba(0,0,0,0.2) !important; /* Ajoute du relief au texte */
        display: flex !important;
        justify-content: center !important;
        align-items: center !important;
    }

    /* Effet au survol pour donner un côté interactif */
   ...
   /* ══ FORMULAIRE DÉMARRAGE ══ */
    ...
    div:not([data-testid="stHorizontalBlock"]) > div > button {
        font-weight: 800 !important;
            ...
            /* Style unifié pour TOUS les boutons de l'application */
        ...
        if isinstance(msg, AIMessage):
            ...
            if matches:
                ...
                calcul_python = (expression_brute
                                 ...
                                 .replace(' ', ''))
                try:
                    # Division → quotient entier + reste (division euclidienne scolaire)
                    if '/' in calcul_python:
                        _m_div = re.match(r'^(\d+)/(\d+)$', calcul_python)
                        if _m_div:
                            _a, _b = int(_m_div.group(1)), int(_m_div.group(2))
                            if _b == 0:
                                continue
                            resultat = _a // _b  # quotient entier
        ...
        if ops[i] in ('*', '/'):
            ...
            if ops[i] == '*':
                ...
                if a % b == 0:
                    ...
                    r = a // b
                    reste = a % b
                    etapes.append(f"{a} ÷ {b} = {r} reste {reste}")
...
def generer_modelage(exercice):
    ...
    Préserve la propriété d'emprunt (soustraction) / retenue (addition) de
    l'exercice original, pour rester cohérent avec l'image affichée.
    ...
    # Propriété à préserver : l'exercice original demande-t-il un emprunt / une retenue ?
    _emprunt_orig = (op == '-' and (a % 10) < (b % 10))      # unités du haut < unités du bas
    _retenue_orig = (op == '+' and ((a % 10) + (b % 10)) >= 10)
    ...
    for _ in range(60):
        ...
        if op == '/' and (nb == 0 or na % nb != 0):
            continue
        # Cœur de l'anti-recopie : résultat différent
        ...
        if (na, nb) == (a, b):
            continue
        # Cohérence avec l'image : même propriété d'emprunt / retenue
        if op == '-' and ((na % 10) < (nb % 10)) != _emprunt_orig:
            continue
        if op == '+' and (((na % 10) + (nb % 10)) >= 10) != _retenue_orig:
            continue
        return f"{na}{sym}{nb}"
    return None
...
def post_traitement(reply, user_input, historique, langue):
    ...
    if gpt_faux:
        exercice = extraire_exercice(historique)
        expr_str = exercice[0] if exercice else ""
        if langue == "العربية":
            return (f"👏 محاولة جيدة ! 😊 لكن الجواب ليس صحيحاً.\n"
                    f"تذكر الصورة ! 😊 إنها مثالٌ — طبّق الخطوات مع أرقامك.\n"
                    f"✏️ حاول مرة أخرى : كم يساوي {expr_str} ؟ 🤔")
        else:
            return (f"👏 Bonne tentative ! 😊 Mais ce n'est pas tout à fait ça.\n"
                    f"Rappelle-toi l'image ! 😊 C'est un exemple — applique les mêmes étapes avec tes chiffres.\n"
                    f"✏️ Essaie encore : Combien font {expr_str} ? 🤔")
...
IMAGES_MAP = {
    "Français": {
        ...
        "tables_1_3_4":              _fr("44_tables_multiplication_1_2_3_4_5.png"),  # tables 1,3,4 (couvert par grille 1-5)
        "tables_6_7_8":              _fr("39_tables_multiplication.png"),  # tables 6,7,8 (couvert par grille 1-9)
        "tables_2_5":                _fr("43_tables_multiplication_2_5.png"),    # CE1 : tables 2 et 5
        "tables_1_2_3_4_5":          _fr("44_tables_multiplication_1_2_3_4_5.png"),  # CE2 : tables 1 à 5
        "multiplication_visuelle":   _fr("45_multiplication_visuelle.png"),       # CE1 : 3×2=6 visuel
        ...
        "multiplication_3ch_1ch":    _fr("42_multiplication_3ch_1ch.png"),       # 245×7=1715
    ...
    "العربية": {
        ...
        "tables_1_3_4":              _ar("44_ar_tables_multiplication_1_2_3_4_5.png"),
        "tables_6_7_8":              _ar("39_ar_tables_multiplication.png"),
        "tables_2_5":                _ar("43_ar_tables_multiplication_2_5.png"),
        "tables_1_2_3_4_5":          _ar("44_ar_tables_multiplication_1_2_3_4_5.png"),
        "multiplication_visuelle":   _ar("45_ar_multiplication_visuelle.png"),
        ...
        "multiplication_3ch_1ch":    _ar("42_ar_multiplication_3ch_1ch.png"),
...
EXPLICATIONS_IMAGES = {
    ...
    "multiplication_3ch_1ch": {
        "Français": "Regarde l'image ! 😊 On multiplie un nombre à 3 chiffres par un seul chiffre. Tu vois les ronds rouges 🔴 ? Ce sont les retenues. On commence par les unités : 5×7=35, on écrit 5 et on retient 3. Puis les dizaines : 4×7=28+3=31, on écrit 1 et on retient 3. Enfin les centaines : 2×7=14+3=17. Résultat : 1715 !",
        "العربية": "انظر للصورة ! 😊 نضرب عدداً من ثلاثة أرقام في رقم واحد. ترى الدوائر الحمراء 🔴 ؟ هذه المحمولات. نبدأ بالآحاد : 5×7=35، نكتب 5 ونحمل 3. ثم العشرات : 4×7=28+3=31، نكتب 1 ونحمل 3. ثم المئات : 2×7=14+3=17. النتيجة : 1715 !"
    },
    ...
    "multiplication_visuelle": {
        "Français": "Regarde l'image ! 😊 Tu vois les 3 groupes de 2 pommes ? Multiplier, c'est compter les groupes pareils ! 3 × 2, ça veut dire 3 fois le nombre 2. On peut aussi additionner : 2 + 2 + 2 = 6. C'est la même chose !",
        "العربية": "انظر للصورة ! 😊 ترى 3 مجموعات من تفاحتين ؟ الضرب هو عدّ المجموعات المتساوية ! 3 × 2 يعني 3 مرات العدد 2. يمكن أيضاً الجمع : 2 + 2 + 2 = 6. إنه نفس الشيء !"
    },
    "tables_2_5": {
        "Français": "Regarde l'image ! 😊 Tu vois les tables de 2 et de 5 ? Pour la table de 2, c'est doubler le nombre. Pour la table de 5, on compte de 5 en 5 : 5, 10, 15, 20... Tu vas vite les retenir !",
        "العربية": "انظر للصورة ! 😊 ترى جدولي 2 و 5 ؟ جدول 2 : ضاعف العدد. جدول 5 : عُد بخمسات : 5، 10، 15، 20... ستحفظهما بسرعة !"
    },
    "tables_1_2_3_4_5": {
        "Français": "Regarde l'image ! 😊 Tu vois les 5 tables faciles ? ×1 = le nombre ne change pas, ×2 = doubler, ×3 = ajouter 3 fois, ×4 = doubler deux fois, ×5 = compter de 5 en 5. Avec ces astuces, c'est facile !",
        "العربية": "انظر للصورة ! 😊 ترى الجداول الخمسة السهلة ؟ ×1 = العدد لا يتغير، ×2 = ضاعف، ×3 = أضف 3 مرات، ×4 = ضاعف مرتين، ×5 = عُد بخمسات. مع هذه النصائح، الأمر سهل !"
    },
    "tables_6_7_8": {
...
CHOIX_OPERATIONS = {
    ...
    "soustraction": {
        ...
        4: [
            ...
            {"label_fr": "Soustraction 3 chiffres − 1 chiffre (125 − 8)", "label_ar": "طرح ثلاثة أرقام − رقم (125 − 8)", "image": "soustraction_3ch_1ch_emprunt",
             "consigne": "soustraction 3 chiffres − 1 chiffre AVEC emprunt, le premier entre 100 et 299"},
            {"label_fr": "Soustraction 3 chiffres − 2 chiffres (345 − 67)", "label_ar": "طرح ثلاثة أرقام − رقمين (345 − 67)", "image": "soustraction_3ch_2ch_emprunt",
             "consigne": "soustraction 3 chiffres − 2 chiffres avec emprunt, le premier entre 100 et 999"},
        ],
        5: [
    ...
    "multiplication": {
        1: [
            {"label_fr": "Multiplication visuelle (3 × 2 = 6)", "label_ar": "الضرب البصري (3 × 2 = 6)", "image": "multiplication_visuelle",
             "consigne": "multiplication visuelle simple, nombres entre 2 et 5, résultat ≤ 25, exemple : 3 × 2 = 6"},
            {"label_fr": "Tables faciles (2 et 5)", "label_ar": "جداول سهلة (2 و 5)", "image": "tables_2_5",
             "consigne": "tables de multiplication 2 et 5 uniquement, produit entre 2×1 et 5×10"},
        ],
        2: [
            {"label_fr": "Tables faciles (1, 2, 3, 4 et 5)", "label_ar": "جداول سهلة (1, 2, 3, 4 و 5)", "image": "tables_1_2_3_4_5",
             "consigne": "tables de multiplication 1, 2, 3, 4 et 5 uniquement, produit entre 1×1 et 5×10"},
        ...
        5: [
            ...
            {"label_fr": "Multiplication à 3 chiffres (245 × 7)", "label_ar": "الضرب بثلاثة أرقام (245 × 7)", "image": "multiplication_3ch_1ch",
             "consigne": "multiplication 3 chiffres × 1 chiffre avec retenues, exemple : 245 × 7 = 1715"},
        ],
        6: [
            {"label_fr": "Multiplication à 2 chiffres (24 × 13)", "label_ar": "الضرب بعددين (24 × 13)", "image": "multiplication_2_chiffres",
             "consigne": "multiplication 2 chiffres × 2 chiffres, deux lignes + addition"},
            {"label_fr": "Multiplication à 3 chiffres (245 × 7 = 1715)", "label_ar": "الضرب بثلاثة أرقام (245 × 7)", "image": "multiplication_3_chiffres",
             "consigne": "multiplication 3 chiffres × 1 chiffre avec retenues, exemple : 245 × 7 = 1715"},
...
def get_image_for_calcul(a, op, b, niveau, langue):
    ...
    if op in ['×', '*', 'x']:
        ...
        if nb_ch_a >= 3 or nb_ch_b >= 3:
            return imgs.get("multiplication_3_chiffres"), "multiplication 3 chiffres × 1 chiffre, exemple : 245 × 7 = 1715"
        ...
        if nb_ch_a <= 1 and nb_ch_b <= 1:
            if ia in [6, 7, 8] or ib in [6, 7, 8]:
                return imgs.get("tables_6_7_8"), "tables de multiplication de 1 à 9, focus sur 6, 7 et 8"
            elif ia in [2, 5, 9] or ib in [2, 5, 9]:
                return imgs.get("tables_2_5_9"), "tables de 2, 5 et 9"
            elif ia in [1, 3, 4] or ib in [1, 3, 4]:
                return imgs.get("tables_1_3_4"), "tables de multiplication de 1 à 5"
            else:
...
# Calculs exemples utilisés dans les images — à ÉVITER dans la génération
EXEMPLES_IMAGES_CALCULS = {
    # addition
    "3+2", "2+3", "23+14", "14+23", "27+15", "15+27", "27+35", "35+27", "357+286", "286+357",
    # soustraction
    "5-2", "48-23", "43-17", "13-7", "124-8", "132-47", "503-247", "834-567", "400-156",
    # multiplication
    "3×2", "2×3", "34×6", "6×34", "24×13", "13×24", "245×7", "7×56", "7×8", "8×7",
    # division
    "84÷4", "47÷5", "156÷12",
}

def generer_calcul_depuis_consigne(operation, consigne, niveau):
    """
    Génère un calcul Python valide (ex: '74-36') à partir de la consigne
    du sous-type dans CHOIX_OPERATIONS.
    Retourne une string ex: '74-36', '7×8', '89÷9', etc.
    """
    import random
    import re
    c = consigne.lower()
    ...
    try:
        if operation == "addition":
            if "3 chiffres" in c:
                a = random.randint(100, 699)
                b = random.randint(100, 899 - a)
                return f"{a}+{b}"
            elif "sans retenue" in c or "sans réten" in c:
                # Unités : a_u + b_u < 10, dizaines libres
                a_d = random.randint(1, 4); a_u = random.randint(1, 8)
                b_d = random.randint(1, 4); b_u = random.randint(1, 9 - a_u)
                return f"{a_d*10+a_u}+{b_d*10+b_u}"
            elif "avec retenue" in c:
                # Unités : a_u + b_u >= 10 ; résultat < 100 (a_d + b_d + 1 < 10)
                a_u = random.randint(3, 9); b_u = random.randint(10 - a_u, 9)
                a_d = random.randint(1, 4); b_d = random.randint(1, 8 - a_d)
                return f"{a_d*10+a_u}+{b_d*10+b_u}"
            else:
            ...
            if "double emprunt" in c or "400" in c:
                a = random.randint(300, 799)
                b = random.randint(100, a - 50)
                return f"{a}-{b}"
            elif "3 chiffres" in c:
                a = random.randint(300, 900)
                b = random.randint(100, a - 50)
                return f"{a}-{b}"
            elif "3 chiffres − 2 chiffres" in c or "3ch_2ch" in c:
                a = random.randint(200, 799)
                b = random.randint(20, 99)
                if a > b: return f"{a}-{b}"
            elif "3 chiffres − 1 chiffre" in c or "3ch_1ch" in c:
                a = random.randint(100, 299)
                b = random.randint(5, 9)
                return f"{a}-{b}"
            elif "2 chiffres − 1 chiffre" in c or "2ch_1ch" in c:
                a = random.randint(20, 50)
                b = random.randint(5, 9)
                return f"{a}-{b}"
            elif "avec emprunt" in c:
                # Unités : a_u < b_u → emprunt nécessaire
                a_u = random.randint(1, 6); b_u = random.randint(a_u + 1, 9)
                a_d = random.randint(3, 8); b_d = random.randint(1, a_d - 1)
                return f"{a_d*10+a_u}-{b_d*10+b_u}"
            elif "sans emprunt" in c:
                a_u = random.randint(3, 9); b_u = random.randint(1, a_u)
                a_d = random.randint(3, 8); b_d = random.randint(1, a_d)
                return f"{a_d*10+a_u}-{b_d*10+b_u}"
            else:
                a = random.randint(4, 9); b = random.randint(1, a - 1)
                return f"{a}-{b}"

        elif operation == "multiplication":
            if "visuelle" in c or "visuel" in c:
                # CE1 : nombres entre 2 et 5, résultat ≤ 25
                # Éviter le calcul de l'exemple (3×2 et 2×3)
                while True:
                    a = random.randint(2, 5); b = random.randint(2, 5)
                    if (a, b) not in [(3, 2), (2, 3)]:
                        break
                return f"{a}×{b}"
            if "1, 2, 3, 4 et 5" in c or "1_2_3_4_5" in c:
                # CE2 : tables 1 à 5
                a = random.randint(1, 5); b = random.randint(1, 10)
                return f"{a}×{b}"
            if ("2 et 5" in c or "tables_2_5" in c) and "2, 5" not in c:
                # CE1 : tables 2 et 5
                a = random.choice([2, 5]); b = random.randint(1, 10)
                return f"{a}×{b}"
            if "10, 100" in c or "10_100" in c:
                a = random.choice([10, 100, 1000])
                ...
                b = random.randint(2, 9)
                return f"{a}×{b}"
            elif "2 chiffres × 2 chiffres" in c:
                a = random.randint(12, 49)
                b = random.randint(11, 29)
                return f"{a}×{b}"
            elif "2 chiffres × 1 chiffre" in c or "2 chiffres" in c:
            ...
            if "2 chiffres" in c:
                ...
                q = random.randint(4, 12)
                r = random.randint(0, b - 1)
                return f"{b*q+r}÷{b}"
            elif "avec reste" in c:
    ...
    # Fallback générique par opération
    fallbacks = {
        "addition": "27+35", "soustraction": "74-36",
        "multiplication": "7×8", "division": "47÷5"
    }
    return fallbacks.get(operation, "5+3")


def _generer_avec_protection(operation, consigne, niveau, max_essais=20):
    """Wrapper qui ré-essaye si le calcul tombe sur un exemple d'image."""
    for _ in range(max_essais):
        calc = generer_calcul_depuis_consigne(operation, consigne, niveau)
        if calc not in EXEMPLES_IMAGES_CALCULS:
            return calc
    return calc  # accepter même un exemple après 20 essais


def choisir_soustype_auto(operation, niveau, user_input=""):
    """
    Choisit automatiquement le sous-type et l'image selon le niveau.
    Si l'élève mentionne un mot-clé précis → détection directe.
    Sinon → choix automatique par niveau.
    Retourne (image_key, consigne) ou (None, None).
    """
    niv_num = int(str(niveau).replace("CE", "")) if niveau else 3
    m = (user_input or "").lower()

    # ── Détection mots-clés précis ──
    if operation == "addition":
        if any(x in m for x in ["3 chiffres", "trois chiffres", "ثلاثة"]):
            return "addition_3_chiffres", "addition à 3 chiffres avec retenues, nombres entre 100 et 999"
        if any(x in m for x in ["retenue", "احتفاظ"]):
            return "addition_avec_retenue", "addition à 2 chiffres AVEC retenue, nombres entre 15 et 89"
        if any(x in m for x in ["sans retenue", "بدون احتفاظ", "simple"]):
            return "addition_simple" if niv_num <= 1 else "addition_sans_retenue",                    "addition simple à 1 chiffre" if niv_num <= 1 else "addition à 2 chiffres SANS retenue"

    elif operation == "soustraction":
        if any(x in m for x in ["double emprunt", "استلافين"]):
            return "soustraction_double_emprunt", "soustraction avec double emprunt, 3 chiffres"
        if any(x in m for x in ["3 chiffres", "trois chiffres"]):
            return "soustraction_3_chiffres", "soustraction à 3 chiffres avec emprunts"
        if any(x in m for x in ["emprunt", "استلاف"]):
            return "soustraction_avec_retenue", "soustraction à 2 chiffres AVEC emprunt, nombres entre 20 et 89"
        if any(x in m for x in ["sans emprunt", "simple", "بدون استلاف"]):
            return ("soustraction_simple", "soustraction simple à 1 chiffre") if niv_num <= 1                    else ("soustraction_sans_retenue", "soustraction à 2 chiffres SANS emprunt")

    elif operation == "multiplication":
        if any(x in m for x in ["visuel", "groupe", "pomme", "bille", "بصري", "مجموعة"]):
            return "multiplication_visuelle", "multiplication visuelle simple, nombres entre 2 et 5, résultat ≤ 25, exemple : 3 × 2 = 6"
        if any(x in m for x in ["10", "100", "1000"]):
            return "multiplication_10_100_1000", "multiplication par 10, 100 ou 1000"
        if any(x in m for x in ["3 chiffres", "trois chiffres"]):
            return "multiplication_3ch_1ch", "multiplication 3 chiffres × 1 chiffre avec retenues"
        if any(x in m for x in ["2 chiffres", "deux chiffres", "posée", "posé"]):
            return "multiplication_2_chiffres", "multiplication 2 chiffres × 2 chiffres"
        if any(x in m for x in ["table", "جدول", "جداول"]):
            # Détection table par mots-clés précis
            if "2 et 5" in m or ("2" in m and "5" in m and "9" not in m and "1" not in m and "3" not in m and "4" not in m):
                return "tables_2_5", "tables de multiplication 2 et 5 uniquement, produit entre 2×1 et 5×10"
            if any(x in m for x in ["facile", "سهل"]) or "1 à 5" in m or "1, 2, 3, 4" in m:
                return "tables_1_2_3_4_5", "tables de multiplication 1, 2, 3, 4 et 5 uniquement, produit entre 1×1 et 5×10"
            if any(x in m for x in ["6", "7", "8"]) and "5" not in m:
                return "tables_6_7_8", "tables de multiplication 6, 7 et 8 uniquement"
            if "2, 5, 9" in m or ("2" in m and "5" in m and "9" in m):
                return "tables_2_5_9", "tables de multiplication 2, 5 et 9 uniquement"
            if "1, 3, 4" in m or ("1" in m and "3" in m and "4" in m):
                return "tables_1_3_4", "tables de multiplication 1, 3 et 4 uniquement"
            return "tables_multiplication", "tables de multiplication de 1 à 9"

    elif operation == "division":
        if any(x in m for x in ["2 chiffres", "deux chiffres"]):
            return "division_2_chiffres", "division posée, diviseur à 2 chiffres"
        if any(x in m for x in ["reste", "الباقي"]):
            return "division_avec_reste", "division avec reste, diviseur 1 chiffre"
        if any(x in m for x in ["simple", "exacte", "مضبوطة", "بدون"]):
            return "division_simple", "division posée, diviseur 1 chiffre, reste = 0"

    # ── Choix automatique par niveau ──
    auto = {
        "addition": {
            1: ("addition_simple",          "addition simple à 1 chiffre, nombres entre 2 et 9"),
            2: ("addition_sans_retenue",    "addition à 2 chiffres SANS retenue, nombres entre 10 et 49"),
            3: ("addition_3_chiffres",      "addition à 3 chiffres avec retenues, nombres entre 100 et 999"),
            4: ("addition_3_chiffres",      "addition à 3 chiffres avec retenues, nombres entre 100 et 999"),
            5: ("addition_3_chiffres",      "addition à 3 chiffres avec retenues, nombres entre 100 et 999"),
            6: ("addition_3_chiffres",      "addition à 3 chiffres avec retenues, nombres entre 100 et 999"),
        },
        "soustraction": {
            1: ("soustraction_simple",        "soustraction simple à 1 chiffre, nombres entre 2 et 9"),
            2: ("soustraction_sans_retenue",  "soustraction à 2 chiffres SANS emprunt, nombres entre 20 et 89, le premier PLUS GRAND"),
            3: ("soustraction_avec_retenue",  "soustraction à 2 chiffres AVEC emprunt, nombres entre 20 et 89"),
            4: ("soustraction_3_chiffres",    "soustraction à 3 chiffres avec emprunts, nombres entre 100 et 999"),
            5: ("soustraction_double_emprunt","soustraction avec double emprunt, 3 chiffres, nombres entre 100 et 999"),
            6: ("soustraction_double_emprunt","soustraction avec double emprunt, 3 chiffres, nombres entre 100 et 999"),
        },
        "multiplication": {
        ...
        "division": {
            3: ("division_simple",      "division posée, diviseur 1 chiffre, reste = 0"),
            4: ("division_avec_reste",  "division avec reste, diviseur 1 chiffre"),
            5: ("division_2_chiffres",  "division posée, diviseur à 2 chiffres"),
            6: ("division_2_chiffres",  "division posée, diviseur à 2 chiffres"),
        },
    }

    niveau_map = auto.get(operation, {})
    if niv_num in niveau_map:
        return niveau_map[niv_num]

    # Fallback : chercher le niveau le plus proche
...
def get_question_clarification(operation, niveau, langue):
    """
    Génère la question de clarification adaptée au niveau.
    Retourne :
    - None        : un seul sous-type → pas de question, passer directement
    - "HORS_NIV:" : opération hors niveau → message à afficher
    - str         : question de clarification normale
    """
    niv_num = int(str(niveau).replace("CE", "")) if niveau else 3
    op_data = CHOIX_OPERATIONS.get(operation, {})

    # Opération absente pour ce niveau → chercher le niveau minimum
    sous_types = op_data.get(niv_num, [])
    if not sous_types:
        niv_min = min(op_data.keys()) if op_data else None
        if niv_min:
            if langue == "العربية":
                noms_ar = {
                ...
                nom = noms_ar.get(operation, operation)
                dispo_ar = {
                    1: "الجمع والطرح",
                    2: "الجمع والطرح والضرب",
                }.get(niv_num, "الجمع والطرح")
                return f"HORS_NIV:فضول رائع ! 😊 {nom} سيأتي لاحقاً. الآن ستصبح بطلاً في {dispo_ar} ! هل تريد أن نبدأ ؟ 💪"
            else:
                noms_fr = {
                ...
                nom = noms_fr.get(operation, operation)
                # Opérations disponibles à ce niveau
                dispo = {
                    1: "addition et soustraction",
                    2: "addition, soustraction et multiplication",
                }.get(niv_num, "addition et soustraction")
                noms_fr2 = {
                    "multiplication": "la multiplication",
                    "division"      : "la division",
                }
                nom_op = noms_fr2.get(operation, nom)
                return f"HORS_NIV:Bonne curiosité ! 😊 {nom_op.capitalize()}, on l'apprend un peu plus tard. Pour l'instant, tu deviens champion en {dispo} ! Tu veux essayer ? 💪"
        return None

    # Un seul sous-type → pas besoin de clarification
    if len(sous_types) <= 1:
        return None
    ...
    return questions.get(operation)
...
def injecter_consigne_etape(message, etape, langue, calcul_force=None):
    """
    ...
    Si calcul_force est fourni (ex: '72-45'), il est imposé pour exercice2/quiz.
    """
    ...
    consignes = {
        ...
        "explication": {
            "Français": "[CONSIGNE SYSTÈME : Tu es en phase EXPLICATION. "
                        ...
                        "Si une image est affichée au-dessus : dis 'Regarde bien l'image ! 😊 C'est un EXEMPLE — applique les mêmes étapes avec TES chiffres.' et ne réexplique PAS la méthode.]",
            "العربية": "[تعليمات النظام : أنت في مرحلة الشرح. "
                       ...
                       "إذا كانت هناك صورة : قل 'انظر للصورة ! 😊 هذه مثال — طبّق نفس الخطوات بأرقامك.' ولا تشرح الطريقة مجدداً.]"
        },
        "exercice1": {
        ...
        "correction1": {
            "Français": "[CONSIGNE SYSTÈME : La réponse est INCORRECTE. "
                        ...
                        "⚠️ Le RÉSULTAT de ton calcul doit être DIFFÉRENT de celui de l'exercice de l'élève. En soustraction, ne décale pas les deux nombres du même montant (pour 35-18, INTERDIT 34-17 : même résultat). "
                        "Exemple : si l'élève bloque sur 126-29, tu résous 143-56=87 devant lui, PAS 126-29. "
                        "TEMPS 3 : Transfert — Retrouve le calcul EXACT posé avec ✏️ dans l'historique et réécris-le mot pour mot. "
                        "Format STRICT : 'À toi de jouer maintenant avec tes chiffres : Combien font [calcul original ✏️] ? 🤔' "
                        "⚠️ INTERDIT ABSOLU : utiliser les chiffres du modelage ou inventer de nouveaux chiffres. "
            ...
            "العربية": "[تعليمات النظام : الإجابة خاطئة. "
                       ...
                       "المرحلة 2 : نمذجة — ❌ ممنوع استخدام أرقام تمرين التلميذ. "
                        ...
                        "⚠️ يجب أن تكون نتيجة حسابك مختلفة عن نتيجة تمرين التلميذ. في الطرح، لا تُزِح العددين بنفس المقدار (مثلاً لـ 35-18، ممنوع 34-17 : نفس النتيجة). "
        ...
        "exercice2": {
            "Français": (f"[CONSIGNE SYSTÈME : Pose l'exercice 2. Écris EXACTEMENT cette question sur une ligne : « ✏️ À toi ! Combien font {calcul_force} ? 🤔 » puis ATTENDS la réponse. N'ajoute aucun autre calcul.]"
                         if calcul_force else
                         "[CONSIGNE SYSTÈME : Pose l'exercice 2 (✏️) : MÊME opération et MÊME type que l'exercice 1, MÊME niveau de difficulté, mais avec des nombres DIFFÉRENTS. Tu DOIS écrire un calcul chiffré concret (ex: « ✏️ À toi ! Combien font 53-27 ? 🤔 »). INTERDIT de changer d'opération. ATTENDS la réponse.]"),
            "العربية": (f"[تعليمات النظام : اطرح التمرين 2. اكتب بالضبط هذا السؤال في سطر واحد : « ✏️ دورك ! كم يساوي {calcul_force} ؟ 🤔 » ثم انتظر الإجابة. لا تضف أي حساب آخر.]"
                        if calcul_force else
                        "[تعليمات النظام : اطرح التمرين 2 (✏️) : نفس العملية ونفس النوع، نفس الصعوبة، بأرقام مختلفة. يجب أن تكتب حساباً رقمياً ملموساً. ممنوع تغيير العملية. انتظر الإجابة.]")
        },
        "quiz": {
            "Français": (f"[CONSIGNE SYSTÈME : Pose le QUIZ. Écris EXACTEMENT cette question sur une ligne : « 🎯 Question : Combien font {calcul_force} ? 😊 » puis ATTENDS la réponse. N'ajoute aucun autre calcul.]"
                         if calcul_force else
                         "[CONSIGNE SYSTÈME : Pose le QUIZ (🎯) — 1 seule question sur la MÊME opération et le MÊME type, difficulté équivalente, nombres DIFFÉRENTS. Tu DOIS écrire un calcul chiffré concret. INTERDIT de changer d'opération. ATTENDS la réponse.]"),
            "العربية": (f"[تعليمات النظام : اطرح الاختبار. اكتب بالضبط هذا السؤال في سطر واحد : « 🎯 سؤال : كم يساوي {calcul_force} ؟ 😊 » ثم انتظر الإجابة. لا تضف أي حساب آخر.]"
                        if calcul_force else
                        "[تعليمات النظام : اطرح الاختبار (🎯) — سؤال واحد فقط، نفس العملية ونفس النوع، صعوبة مماثلة، أرقام مختلفة. يجب أن تكتب حساباً رقمياً ملموساً. ممنوع تغيير العملية. انتظر الإجابة.]")
        },
        "felicitations": {
...
→ Si INCORRECT — SÉQUENCE OBLIGATOIRE EN 4 TEMPS :
   ...
   ⚠️ RÈGLE ANTI-RECOPIE : le RÉSULTAT de ton calcul inventé doit être DIFFÉRENT du résultat du calcul original de l'élève.
   En soustraction, ne décale JAMAIS les deux nombres du même montant (ex : pour 35-18, INTERDIT de proposer 34-17 ou 36-19 : leur résultat est identique et l'élève pourrait recopier sans calculer).
   Avant d'écrire, VÉRIFIE : si ton calcul donne le même résultat que celui de l'élève, change les nombres.
   ...
   ⚠️ INTERDIT ABSOLU : [a] et [b] sont UNIQUEMENT les chiffres du calcul original marqué ✏️ dans l'historique.
   ❌ JAMAIS les chiffres du modelage que tu viens d'inventer.
   ❌ JAMAIS de nouveaux chiffres inventés.
   Si tu n'es pas sûr, relis l'historique et cherche le dernier ✏️.
...
if "attente_oui_hors_niveau" not in st.session_state: st.session_state["attente_oui_hors_niveau"] = False
...
if not chat_actif:
    ...
    if langue_choisie == "Français":
        titre_html = (
            "<span style=\"font-size:1.1rem; font-weight:500; color:white; line-height:1.4;\">👋 Bienvenue ! Je suis ton tuteur IA en mathématiques.</span><br>"
            "<span style=\"font-size:0.95rem; font-weight:400; color:white; opacity:0.95; line-height:1.5;\">"
            "Écris ton <span style=\"color:#FFD93D; font-weight:500;\">prénom</span>, "
            "choisis ton <span style=\"color:#FFD93D; font-weight:500;\">niveau</span>, "
            "puis clique sur <span style=\"color:#FFD93D; font-weight:500;\">Commencer</span> pour débuter !"
            "</span>"
        )
    else:
        titre_html = (
            "<span style=\"font-size:1.1rem; font-weight:500; color:white; line-height:1.4;\">👋 مرحباً بك! أنا معلمك الذكي في الرياضيات.</span><br>"
            "<span style=\"font-size:0.95rem; font-weight:400; color:white; opacity:0.95; line-height:1.5;\">"
            "اكتب <span style=\"color:#FFD93D; font-weight:500;\">اسمك</span>، "
            "اختر <span style=\"color:#FFD93D; font-weight:500;\">مستواك</span>، "
            "ثم اضغط على <span style=\"color:#FFD93D; font-weight:500;\">ابدأ</span> للبدء!"
            "</span>"
        )
    ...
    with col_p:
        # Titre stylisé pour le prénom
        titre_prenom = "👤 Quel est ton prénom ?" if langue_choisie == "Français" else "👤 ما هو اسمك؟"
        st.markdown(f"<p style='font-family: Fredoka One; color: #000000; font-size: 0.9rem; margin-bottom: 5px;'>{titre_prenom}</p>", unsafe_allow_html=True)
    ...
    with col_n:
        # Titre stylisé pour le niveau
        titre_niveau = "📚 Quel est ton niveau ?" if langue_choisie == "Français" else "📚 ما هو مستواك؟"
        st.markdown(f"<p style='font-family: Fredoka One; color: #000000; font-size: 0.9rem; margin-bottom: 5px;'>{titre_niveau}</p>", unsafe_allow_html=True)
        
        _ph = "Choisis ton niveau ici" if langue_choisie == "Français" else "اختر مستواك هنا"
        niveau_input = st.selectbox(
            lbl_niveau, 
            [""] + NIVEAUX_LIST, 
            key=_niv_key,
            label_visibility="collapsed", # On cache le label par défaut
...
def _render_md_to_html(text):
    """Convertit **gras** et sauts de ligne en HTML pour affichage dans une div."""
    import re
...
for i, msg in enumerate(chat_history):
    ...
    with st.chat_message(role):
        if role == "assistant":
            ...
            _html = _render_md_to_html(msg.content)
            st.markdown(f'<div dir="{direction}">{_html}</div>', unsafe_allow_html=True)
        else:
            _html = _render_md_to_html(msg.content)
            st.markdown(f'<div dir="{direction}">{_html}</div>', unsafe_allow_html=True)
    ...
    if user_input:
        ...
        if est_changement:
            ...
            if _new_operation and not any(c.isdigit() for c in user_input):
                ...
                # Option C : Python choisit automatiquement le sous-type
                _hn_check = get_question_clarification(_new_operation, niveau_eleve, langue_choisie)
                if _hn_check and _hn_check.startswith("HORS_NIV:"):
                    texte_hn = _hn_check.replace("HORS_NIV:", "")
                    direction = "rtl" if langue_choisie == "العربية" else "ltr"
                    with st.chat_message("assistant"):
                        st.markdown(f'<div dir="{direction}">{texte_hn}</div>', unsafe_allow_html=True)
                    st.session_state[session_key].append(HumanMessage(content=user_input))
                    st.session_state[session_key].append(AIMessage(content=texte_hn))
                    try:
                        sid_db = st.session_state[eleve_key].get("session_db_id")
                        db_ajouter_message(sid_db, "eleve", user_input)
                        db_ajouter_message(sid_db, "tuteur", texte_hn)
                    except Exception:
                        pass
                    st.rerun()
                else:
                    _img_key_auto, _consigne_auto = choisir_soustype_auto(_new_operation, niveau_eleve, user_input)
                    imgs_auto = IMAGES_MAP.get(langue_choisie, IMAGES_MAP["Français"])
                    _img_path_auto = imgs_auto.get(_img_key_auto) if _img_key_auto else None
                    if _consigne_auto: st.session_state["choix_consigne"] = _consigne_auto
                    if _img_path_auto and os.path.exists(_img_path_auto):
                        st.session_state["choix_image"] = _img_path_auto
                    else:
                        st.session_state["choix_image"] = None
                    st.session_state["hors_niveau_operation"] = _new_operation
                    st.session_state["derniere_operation"] = _new_operation
                    st.session_state[etape_key] = "explication"
                    etape_actuelle = "explication"
                
            elif not _new_operation and veut_changer_explicitement:
                direction = "rtl" if langue_choisie == "العربية" else "ltr"
                if langue_choisie == "العربية":
                    _msg_amorce = f"بالتأكيد ! 😊 ماذا تريد أن تتعلم الآن {prenom_eleve} ؟\nالجمع ➕ الطرح ➖ الضرب ✖️ القسمة ➗"
                else:
                    _msg_amorce = f"Pas de problème ! 😊 Qu'est-ce que tu veux apprendre maintenant {prenom_eleve} ?\nAddition ➕  Soustraction ➖  Multiplication ✖️  Division ➗"
                with st.chat_message("assistant"):
                    st.markdown(f'<div dir="{direction}">{_msg_amorce}</div>', unsafe_allow_html=True)
                st.session_state[session_key].append(HumanMessage(content=user_input))
                st.session_state[session_key].append(AIMessage(content=_msg_amorce))
                try:
                    sid_db = st.session_state[eleve_key].get("session_db_id")
                    db_ajouter_message(sid_db, "eleve", user_input)
                    db_ajouter_message(sid_db, "tuteur", _msg_amorce)
                except Exception:
                    pass
                st.rerun()
        ...
        def _detecter_op_locale(msg):
            ...
            return None
        # ── Détection "oui" après message HORS_NIV ──
        _ui_lower = user_input.strip().lower()
        if st.session_state.get("attente_oui_hors_niveau") and _ui_lower in ("oui", "ok", "d\'accord", "yes", "نعم", "موافق", "اوك", "حسناً"):
            st.session_state["attente_oui_hors_niveau"] = False
            # Proposer la première opération disponible pour le niveau
            niv_num_o = int(str(niveau_eleve).replace("CE", "")) if niveau_eleve else 3
            premiere_op = "addition"  # par défaut
            for op_test in ["addition", "soustraction", "multiplication", "division"]:
                if CHOIX_OPERATIONS.get(op_test, {}).get(niv_num_o):
                    premiere_op = op_test
                    break
            _img_key_o, _consigne_o = choisir_soustype_auto(premiere_op, niveau_eleve, "")
            imgs_o = IMAGES_MAP.get(langue_choisie, IMAGES_MAP["Français"])
            _img_path_o = imgs_o.get(_img_key_o) if _img_key_o else None
            if _consigne_o: st.session_state["choix_consigne"] = _consigne_o
            if _img_path_o and os.path.exists(_img_path_o):
                st.session_state["choix_image"] = _img_path_o
            else:
                st.session_state["choix_image"] = None
            st.session_state["hors_niveau_operation"] = premiere_op
            st.session_state["derniere_operation"] = premiere_op
            st.session_state[etape_key] = "explication"
            etape_actuelle = "explication"
        ...
        if (_operation_universelle
            ...
            # Option C : Python choisit automatiquement
            _hn_check_u = get_question_clarification(_operation_universelle, niveau_eleve, langue_choisie)
            if _hn_check_u and _hn_check_u.startswith("HORS_NIV:"):
                texte_hn = _hn_check_u.replace("HORS_NIV:", "")
                direction = "rtl" if langue_choisie == "العربية" else "ltr"
                with st.chat_message("assistant"):
                    st.markdown(f'<div dir="{direction}">{texte_hn}</div>', unsafe_allow_html=True)
                st.session_state[session_key].append(HumanMessage(content=user_input))
                st.session_state[session_key].append(AIMessage(content=texte_hn))
                st.session_state["attente_oui_hors_niveau"] = True
                try:
                    sid_db = st.session_state[eleve_key].get("session_db_id")
                    db_ajouter_message(sid_db, "eleve", user_input)
                    db_ajouter_message(sid_db, "tuteur", texte_hn)
                ...
                _img_key_u, _consigne_u = choisir_soustype_auto(_operation_universelle, niveau_eleve, user_input)
                imgs_u = IMAGES_MAP.get(langue_choisie, IMAGES_MAP["Français"])
                _img_path_u = imgs_u.get(_img_key_u) if _img_key_u else None
                if _consigne_u: st.session_state["choix_consigne"] = _consigne_u
                if _img_path_u and os.path.exists(_img_path_u):
                    st.session_state["choix_image"] = _img_path_u
                else:
                    st.session_state["choix_image"] = None
                st.session_state["hors_niveau_operation"] = _operation_universelle
                st.session_state["derniere_operation"] = _operation_universelle
                st.session_state[etape_key] = "explication"
                etape_actuelle = "explication"
        _skip_gpt = False
        if etape_actuelle == "amorce":
            ...
            if calcul:
                ...
                if _pas_de_chiffres:
                    ...
                    _hn_check_m = get_question_clarification(operation, niveau_eleve, langue_choisie)
                    if _hn_check_m and _hn_check_m.startswith("HORS_NIV:"):
                        texte_hn = _hn_check_m.replace("HORS_NIV:", "")
                        direction = "rtl" if langue_choisie == "العربية" else "ltr"
                        with st.chat_message("assistant"):
                            st.markdown(f'<div dir="{direction}">{texte_hn}</div>', unsafe_allow_html=True)
                        st.session_state[session_key].append(HumanMessage(content=user_input))
                        ...
                        _img_key_m, _consigne_m = choisir_soustype_auto(operation, niveau_eleve, user_input)
                        imgs_m = IMAGES_MAP.get(langue_choisie, IMAGES_MAP["Français"])
                        _img_path_m = imgs_m.get(_img_key_m) if _img_key_m else None
                        if _consigne_m: st.session_state["choix_consigne"] = _consigne_m
                        if _img_path_m and os.path.exists(_img_path_m):
                            st.session_state["choix_image"] = _img_path_m
                        else:
                            st.session_state["choix_image"] = None
                        st.session_state["hors_niveau_operation"] = operation
                        st.session_state["derniere_operation"] = operation
                        st.session_state[etape_key] = "explication"
                        etape_actuelle = "explication"
                        _clarif_msg = ""

                    # Injecter directement dans le chat sans appeler GPT (seulement si message non vide)
                    if _clarif_msg:
                        direction = "rtl" if langue_choisie == "العربية" else "ltr"
                        with st.chat_message("assistant"):
                        ...
                        try:
                            ...
                            pass
                        _skip_gpt = True
                    # Si _clarif_msg vide → Option C → laisser GPT continuer normalement
                ...
                # ── CAS 3 : Problème énoncé avec chiffres → image précise via calcul extrait ──
                    ...
                    if not detecter_probleme_enonce(user_input):
                        st.session_state["hors_niveau_operation"] = operation
                        st.session_state[etape_key] = "clarification"
                        _hn_check_c3 = get_question_clarification(operation, niveau_eleve, langue_choisie)
                        if _hn_check_c3 and _hn_check_c3.startswith("HORS_NIV:"):
                            texte_hn = _hn_check_c3.replace("HORS_NIV:", "")
                            direction = "rtl" if langue_choisie == "العربية" else "ltr"
                            with st.chat_message("assistant"):
                                st.markdown(f'<div dir="{direction}">{texte_hn}</div>', unsafe_allow_html=True)
                            st.session_state[session_key].append(HumanMessage(content=user_input))
                            ...
                            _img_key_c3, _consigne_c3 = choisir_soustype_auto(operation, niveau_eleve, user_input)
                            imgs_c3 = IMAGES_MAP.get(langue_choisie, IMAGES_MAP["Français"])
                            _img_path_c3 = imgs_c3.get(_img_key_c3) if _img_key_c3 else None
                            if _consigne_c3: st.session_state["choix_consigne"] = _consigne_c3
                            if _img_path_c3 and os.path.exists(_img_path_c3):
                                st.session_state["choix_image"] = _img_path_c3
                            else:
                                st.session_state["choix_image"] = None
                            st.session_state["hors_niveau_operation"] = operation
                            st.session_state["derniere_operation"] = operation
                        ...
                        if _clarif_c3:
                            ...
                            try:
                                ...
                                pass
                            st.rerun()
                        # Si _clarif_c3 vide → Option C → laisser GPT continuer
                    else:
                    ...
                    st.session_state["derniere_operation"] = operation  # cohérence enchaînement
                    st.session_state[etape_key] = "explication"
                    etape_actuelle = "explication"
        # ── CAS 4 : Réponse à la clarification ──
            ...
            st.session_state["derniere_operation"] = stored_op  # sauvegarde pour generer_calcul
            ...
            with st.chat_message("assistant"):
                with st.spinner(t["thinking"]):
                    try:
                        ...
                        # Si pas de calcul direct → Python génère un vrai calcul depuis CHOIX_OPERATIONS
                        if not _calcul_direct and _choix_consigne:
                            _op_pour_gen = (
                                st.session_state.get("hors_niveau_operation")
                                or st.session_state.get("derniere_operation")
                                or st.session_state.get("operation_detectee")
                                or ""
                            )
                            _niv_num_gen = int(str(niveau_eleve).replace("CE", "")) if niveau_eleve else 3
                            _calcul_direct = _generer_avec_protection(
                                _op_pour_gen, _choix_consigne, _niv_num_gen
                            )
                        _exercice_final    = f"{_calcul_direct}" if _calcul_direct else "5+3"
                        _exercice_final_ar = f"{_calcul_direct}" if _calcul_direct else "5+3"
                        if _img_path and _choix_consigne and etape_actuelle in ("explication", "choix_sujet"):
                            ...
                            if langue_choisie == "العربية":
                                consigne_specifique = (
                                    ...
                                    f"2) بعد الشرح، قل حتماً : 'هذه مثال — طبّق نفس الخطوات بأرقامك {_exercice_final_ar} ! 😊'\n"
                                    f"3) ❌ ممنوع منعاً باتاً : لا تقم بحل العملية ولا تعطِ النتيجة النهائية أبداً.\n"
                                    f"4) اختم بـ : '✏️ دورك ! كم يساوي {_exercice_final_ar} ؟ 🤔' — استخدم هذه الأرقام بالضبط. ]"
                                )
                            else:
                                consigne_specifique = (
                                    ...
                                    f"« {explication_statique} »\n"
                                    f"2) Après l'explication, dis OBLIGATOIREMENT : 'C'est un EXEMPLE — applique les mêmes étapes avec TES chiffres {_exercice_final} ! 😊'\n"
                                    f"3) ❌ INTERDIT ABSOLU : NE RÉSOUD PAS le calcul et NE DONNE JAMAIS le résultat final.\n"
                                    f"4) Termine OBLIGATOIREMENT par : '✏️ À toi ! Combien font {_exercice_final} ? 🤔']\""
                                )
                            ...
                            elif _verdict_pre and isinstance(_verdict_pre, str) and _verdict_pre.startswith("correct"):
                                # RÉPONSE CORRECTE → faire avancer la consigne vers l'étape SUIVANTE
                                if etape_actuelle in ("exercice1", "correction1"):
                                    etape_pour_consigne = "exercice2"
                                elif etape_actuelle in ("exercice2", "correction2"):
                                    etape_pour_consigne = "quiz"
                                elif etape_actuelle in ("quiz", "correction_quiz"):
                                    etape_pour_consigne = "felicitations"
                            # Pour exercice2 et quiz : imposer un calcul généré par Python
                            _calcul_force = None
                            if etape_pour_consigne in ("exercice2", "quiz"):
                                try:
                                    # Extraire le calcul de la DERNIÈRE question posée (fiable)
                                    _exo_ref = ""
                                    for _m in reversed(chat_history):
                                        if isinstance(_m, AIMessage):
                                            _txt = _m.content.replace('−', '-')
                                            _q = re.search(r'(?:Combien font|كم يساوي|Question\s*:)\s*(\d+\s*[+\-*/×÷]\s*\d+)', _txt)
                                            if _q:
                                                _exo_ref = _q.group(1).replace(' ', '')
                                                break
                                    if "-" in _exo_ref:
                                        _op_ref = "soustraction"
                                    elif "×" in _exo_ref or "*" in _exo_ref:
                                        _op_ref = "multiplication"
                                    elif "÷" in _exo_ref or "/" in _exo_ref:
                                        _op_ref = "division"
                                    elif "+" in _exo_ref:
                                        _op_ref = "addition"
                                    else:
                                        _op_ref = (st.session_state.get("derniere_operation")
                                                   or st.session_state.get("operation_detectee") or "addition")
                                    _niv_ref = st.session_state[eleve_key].get("niveau", "CE3")
                                    _, _cons_ref = choisir_soustype_auto(_op_ref, _niv_ref)
                                    def _res_calc(expr):
                                        try:
                                            ...
                                            return None
                                    _res_used = st.session_state.get("results_used", set())
                                    if _res_calc(_exo_ref) is not None:
                                        _res_used.add(_res_calc(_exo_ref))
                                    _calcul_force = None
                                    for _ in range(25):
                                        _cand = generer_modelage((_exo_ref,)) if _exo_ref else None
                                        if not _cand:
                                            _cand = _generer_avec_protection(_op_ref, _cons_ref or "", _niv_ref)
                                        if _res_calc(_cand) not in _res_used:
                                            _calcul_force = _cand
                                            break
                                        _calcul_force = _cand
                                    if _res_calc(_calcul_force) is not None:
                                        _res_used.add(_res_calc(_calcul_force))
                                    st.session_state["results_used"] = _res_used
                                except Exception:
                                    _calcul_force = None
                            message_final = injecter_consigne_etape(
                                message_avec_verdict, etape_pour_consigne, langue_choisie, _calcul_force
                            )
                        ...
                        if _verdict is not None:
                            ...
                            if _verdict == "correct" or (isinstance(_verdict, str) and _verdict.startswith("correct")):
                                ...
                                if any(m in assistant_reply.lower() for m in mots_negatifs):
                                    ...
                                    # Générer DIRECTEMENT le nouvel exercice (pas de pause)
                                    try:
                                        # Source de vérité prioritaire : l'exercice réellement en cours
                                        _exo_cur = (extraire_exercice(chat_history) or "").replace('−', '-')
                                        if "-" in _exo_cur:
                                            _op_cur = "soustraction"
                                        elif "×" in _exo_cur or "*" in _exo_cur:
                                            _op_cur = "multiplication"
                                        elif "÷" in _exo_cur or "/" in _exo_cur:
                                            _op_cur = "division"
                                        elif "+" in _exo_cur:
                                            _op_cur = "addition"
                                        else:
                                            # Pas d'exercice lisible → repli sur l'état mémorisé
                                            _op_cur = (st.session_state.get("derniere_operation")
                                                       or st.session_state.get("hors_niveau_operation")
                                                       or "addition")
                                        _niv_cur = st.session_state[eleve_key].get("niveau", "CE3")
                                        _consigne_cur = st.session_state.get("choix_consigne") or ""
                                        if not _consigne_cur:
                                            # Consigne perdue → la régénérer via la sélection adaptative par niveau
                                            _, _consigne_cur = choisir_soustype_auto(_op_cur, _niv_cur)
                                            _consigne_cur = _consigne_cur or ""
                                        nouveau_calc = _generer_avec_protection(_op_cur, _consigne_cur, _niv_cur)
                                    except Exception:
                                        nouveau_calc = None
                                    if nouveau_calc:
                                        if langue_choisie == "العربية":
                                            assistant_reply = f"🌟 أحسنت {prenom} ! الجواب صحيح ! 💪\n✏️ دورك الآن ! كم يساوي {nouveau_calc} ؟ 🤔"
                                        else:
                                            assistant_reply = f"🌟 Bravo {prenom} ! Tu as trouvé la bonne réponse ! 💪\n✏️ À toi ! Combien font {nouveau_calc} ? 🤔"
                                    else:
                                        if langue_choisie == "العربية":
                                            assistant_reply = f"🌟 أحسنت {prenom} ! الجواب صحيح ! 💪\n✏️ نواصل ! 😊"
                                        else:
                        ...
                        if etape_av == "felicitations":
                            ...
                            if msg_strip in ["2", "٢", "2️⃣"] or "autre" in ul or "آخر" in ul or "chang" in ul or "sujet" in ul or "موضوع" in ul or "غير" in ul:
                                st.session_state[etape_key]      = "amorce"
                                st.session_state["bonnes_consec"] = 0
                                st.session_state["choix_image"]   = None
                                st.session_state["choix_consigne"] = None
                                st.session_state["results_used"]  = set()
                                if langue_choisie == "العربية":
