...
(function() {{
    function styleBtns() {{
        ...
        doc.querySelectorAll('button').forEach(function(btn) {{
            ...
            // 1. On tue la largeur minimale forcée par Streamlit
            var wrapper = btn.closest('div[data-testid="stButton"], div[data-testid="stDownloadButton"]');
            if (wrapper) {{
                wrapper.style.setProperty('width', '100%', 'important');
                wrapper.style.setProperty('min-width', '0', 'important'); /* CRUCIAL */
                wrapper.style.setProperty('margin', '0', 'important');
            }}

            // 2. Boutons capables de rétrécir à l'extrême
            btn.setAttribute('style',
            ...
            // 3. On force le texte interne à obéir
            btn.querySelectorAll('p, span, div').forEach(function(el) {{
                el.style.cssText = 'margin:0 !important;padding:0 !important;text-align:center !important;width:100% !important;color:white !important;font-size:inherit !important;overflow:hidden !important;text-overflow:ellipsis !important;white-space:nowrap !important;';
            }});
        ...
        // 4. LA GRILLE IMPÉNÉTRABLE
        if (targetRow) {{
            targetRow.style.setProperty('display', 'grid', 'important');
            /* L'ARME ULTIME : 2 colonnes qui peuvent rétrécir jusqu'à 0 pixel si besoin */
            targetRow.style.setProperty('grid-template-columns', 'minmax(0, 1fr) minmax(0, 1fr)', 'important'); 
            targetRow.style.setProperty('gap', '8px', 'important'); /* Espace au centre */
            ...
            targetRow.style.setProperty('padding', '0 15px', 'important'); /* Marge droite/gauche pour décoller du bord */
            
            var cols = targetRow.querySelectorAll('div[data-testid="column"]');
            cols.forEach(function(c) {{
                c.style.setProperty('width', '100%', 'important');
                c.style.setProperty('min-width', '0', 'important'); /* Enlève la sécurité de Streamlit */
