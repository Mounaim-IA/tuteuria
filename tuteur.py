...
def injecter_etapes_expression(message, langue):
    ...
    Détecte aussi l'opération pour choisir l'image adaptée.
    """
    ...
    try:
        ...
        # ── Détecter l'opération dominante pour l'image ──
        if   '*' in calc:            op_type = "multiplication"
        elif '/' in calc:            op_type = "division"
        elif '-' in calc and '+' not in calc: op_type = "soustraction"
        elif '+' in calc and '-' not in calc: op_type = "addition"
        else:                         op_type = "mixte"
        if langue == "العربية":
            return (f"{message}\n[CALCUL PYTHON VÉRIFIÉ ✅ — تعبير حر\n"
                    ...
                    f"اشرح هذه الخطوات بالرموز ثم اختم بـ '✏️ دورك !' وتمرين مختلف الأرقام.]")
        else:
    ...
    with st.chat_message("assistant"):
        with st.spinner(t["thinking"]):
            try:
                ...
                # ── 1b. Image pour expression libre (ex: "34-17") ──
                if (not st.session_state.get("choix_image")
                        and est_nouvelle_expression(user_input)
                        and verifier_reponse(user_input, chat_history) is None):
                    _expr_c = re.sub(r'[^0-9+\-*/]', '', user_input.replace('÷','/').replace('×','*').replace('x','*'))
                    _niv_e = st.session_state[eleve_key].get("niveau", "CE3")
                    _imgs_e = IMAGES_MAP.get(langue_choisie, IMAGES_MAP["Français"])
                    _n = int(_niv_e.replace("CE","")) if "CE" in str(_niv_e) else 3
                    if '*' in _expr_c:
                        _k = "multiplication_simple" if _n <= 3 else "multiplication_2_chiffres"
                    elif '/' in _expr_c:
                        _k = "division_avec_reste" if _n <= 4 else "division_decimale"
                    elif '-' in _expr_c and '+' not in _expr_c:
                        _k = ("soustraction_simple" if _n <= 1 else
                              "soustraction_sans_retenue" if _n == 2 else
                              "soustraction_avec_retenue" if _n == 3 else
                              "soustraction_3_chiffres" if _n == 4 else "soustraction_decimaux")
                    elif '+' in _expr_c and '-' not in _expr_c:
                        _k = ("addition_simple" if _n <= 1 else
                              "addition_sans_retenue" if _n == 2 else
                              "addition_avec_retenue" if _n == 3 else
                              "addition_3_chiffres" if _n == 4 else "addition_decimaux")
                    else:
                        _k = None
                    if _k:
                        _img_e = _imgs_e.get(_k)
                        if _img_e and os.path.exists(_img_e):
                            st.session_state["choix_image"] = _img_e
                ...
                if _img_path and _choix_consigne and etape_actuelle == "explication":
                    ...
                    # Garder _img_path si expression libre (détectée en step 1b)
                    # Sinon reset (exercice/correction/quiz → pas d'image)
                    _is_libre = (est_nouvelle_expression(user_input)
                                 and verifier_reponse(user_input, chat_history) is None)
                    if not _is_libre:
                        _img_path = None
                    # Nettoyer choix_image pour ne pas l'afficher au prochain message
                    if _is_libre:
                        st.session_state["choix_image"] = None
