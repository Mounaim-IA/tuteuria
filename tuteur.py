    ...
    with st.chat_message("assistant"):
        with st.spinner(t["thinking"]):
            try:
                    ...
                    if langue_choisie == "العربية":
                        ...
                        if is_pb:
                            consigne_specifique = (
                                ...
                                f"Ligne 2 : 'Regarde l'image ci-dessous 👇 elle te montre comment faire'\n"
                                f"[IMAGE]\n"
                                f"Ligne 3-4 : Explique l'exemple de l'image avec SES chiffres : {_choix_consigne}\n"
                            ...
                            consigne_specifique = (
                                ...
                                f"Ligne 2 : 'Regarde l'image ci-dessous 👇 elle te montre la méthode'\n"
                                f"[IMAGE]\n"
                                f"Ligne 3-4 : Explique l'exemple de l'image : ligne 1 = unités × tout, ligne 2 = dizaines × tout + point •, puis addition. Chiffres de l'image : {_choix_consigne}\n"
                            ...
                            consigne_specifique = (
                                ...
                                f"Ligne 2 : 'Regarde l'image ci-dessous 👇 elle te montre comment faire'\n"
                                f"[IMAGE]\n"
                                f"Ligne 3-4 : Explique l'exemple de l'image en 1-2 phrases avec SES chiffres uniquement : {_choix_consigne}\n"
                ...
                # ── Affichage : intro → image → explication + exercice ──
                # GPT génère le texte avec [IMAGE] comme séparateur
                if _img_path and "[IMAGE]" in assistant_reply:
                    parties = assistant_reply.split("[IMAGE]", 1)
                    intro   = parties[0].strip()
                    suite   = parties[1].strip() if len(parties) > 1 else ""
                    if intro:
                        st.markdown(f'<div dir="{direction}">{intro}</div>', unsafe_allow_html=True)
                    st.image(_img_path, use_container_width=True)
                    if suite:
                        st.markdown(f'<div dir="{direction}">{suite}</div>', unsafe_allow_html=True)
                    # Stocker la version complète sans [IMAGE] dans l'historique
                    assistant_reply = f"{intro}\n{suite}".strip()
                elif _img_path:
                    # Pas de séparateur → image avant texte (comportement par défaut)
                    st.image(_img_path, use_container_width=True)
