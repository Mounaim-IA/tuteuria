...
(function() {{
    function styleBtns() {{
        ...
        doc.querySelectorAll('button').forEach(function(btn) {{
            ...
            // Padding réduit et texte ajusté pour les petits écrans
            btn.setAttribute('style',
                ...
                'align-items:center !important;' +
                'justify-content:center !important;' +
                'padding:6px 2px !important;' + /* PADDING RÉDUIT ICI */
                'width:100% !important;' +
        ...
        if (targetBlock) {{
            ...
            targetBlock.style.setProperty('gap', '6px', 'important'); /* ESPACE RÉDUIT ENTRE LES BOUTONS */
            targetBlock.style.setProperty('width', '100%', 'important');
            targetBlock.style.setProperty('max-width', '100vw', 'important'); /* INTERDIT LE DÉPASSEMENT DE L'ÉCRAN */
            targetBlock.style.setProperty('box-sizing', 'border-box', 'important');
            
            var cols = targetBlock.querySelectorAll('div[data-testid="column"]');
            cols.forEach(c => {{
                // La magie opère ici : au lieu de 50%, on utilise flex: 1 pour calculer la taille parfaite
